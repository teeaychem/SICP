...
Here we'll have six enviroments, the first containg ~prodcut: 1, counter: 1, max-count: 6~, the second ~prodcut: 1, counter: 2, max-count: 6~, and the last ~prodcut: 120, counter: 6, max-count: 6~.
...
This takes constant space, as the only additional thing is the marker.
...
First procedure is ~insert-crumb~.
...
Second, we have ~insert-crumb~, which removes a crumb so long as it's where it was inserted.
...
Next up is ~push-crumb~ which pushes the crumb one space forward in the list.
...
At the moment, there's no way to check the first element of the list.
However, the above procedures can be modified to move the crumb appropriately, so long as the relevant structure is present.

And, there is an issue of returning to where the crumb is.
...
I *think* this covers everything, and typing everything else seems like an interseting challenge!
