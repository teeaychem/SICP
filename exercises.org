...
It's a little messy, though today it's too late for improvements.

In short, we set up a list for the terms, and a pointer to the last element of the list in order to easily add new elements.
Then, the core of the procedure is the 'destructive' ~collect-replace~.
This adds elements to the list, and rewrites the body.
We apply this to each expression in the body by ~map~.
As ~collect-replace~ builds the term list, we store the output and then make a call to ~make-let~.

Could parse and replace separately, but that's two scans of the body.
So, while conceptually nice, is fairly inefficient.

Could also do something more involved for procedure definitions, as these are rewritten.
Though, I don't see this as expected, and it's not super interesting.

Update: I'm not clear on whether this /actually/ works.
The output is as expected in tests, but it causes some issues if applied to any body.
For the moment, this is modified to only make the transformation if an internal definiton is found.
Update 2: The modification ensured the proc always retruned the body.
This is fixed by explicitly setting a flag for whether a definition has been found.
Also, commented option to allow application to a single definition.
()
