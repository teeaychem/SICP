#+STARTUP: overview
#+PROPERTY: header-args :prologue "#lang sicp" :noweb eval :results output :tangle test.rkt
...
#+BEGIN_SRC racket
	...
	(eq? (car object) 'leaf))
      ...
      (define (left-branch-tree-hf tree) (car tree))

      (define (right-branch-tree-hf tree) (cadr tree))

      (define (symbols-hf tree)
	(if (leaf?-hf tree)
	    (list (symbol-leaf-hf tree)) ; leaf, so 2nd elem, but make into a list on one
