...
And, any list which contains at least one element is a pair.
See:
...
Well, it's not clear what we're testing for.
What exactly does ~(cond-actions (test => recipient))~ return?
Is it ~(cond-actions (=> recipient))~?
If so, is this a list?
Is there any syntactic sugar involved?

The place to look is ~sequence->exp~.
For, this is where the evaluation takes place.
We'd like to hijack this to first test for this expression, and then proceed as before.

For example, we'd have something like:

#+BEGIN_SRC racket
  (define (hijack-sequence->exp predicate actions)
    (if (arrow? actions)
	(sequence-exp (apply (arrow-end actions) (eval predicate)))
	(sequence-exp actions)))
#+END_SRC

And, then replace the calls to ~sequence->exp~ with:

#+BEGIN_SRC racket
  (hijack-sequence->exp (cond-predicate first) (cond-actions first))
#+END_SRC

I would expect ~arrow?~ tests for a tagged list with the tag ~'=>~, and then ~arrow-end~ would just be ~cadr~.
But, hard to be sure.
(Is this the point at the moment?)

Wait.
There's an issue here.
~make-if~ leads to an instance of ~eval-if~, and in turn this leads to evaluating the predicate.
And, the first instance of ~sequence->exp~ triggers on an ~else~ clause, so the ~=>~ does not obtain, for sure.

Still, why wait to evaluate the predicate?
We can do it now! (edit: this seems misguided, as we'd want to take an env argument, rather than assuming the argument is always called in the scope of some env --- in particular, this is bad if the variable gets renamed.)
But, that's going to cause an issue.
At the moment, we're trying to transform ~cond~ into nested ~if~.
It seems like a bad idea to evaluate the predicate.
As, for example, the particular ~cond~ instance may only eval given the failure of previous instances.

It would be nice to memo the predicate with ~delay~.
But, we'd then also need to ~force~ the result somewhere.

So, okay, without redefining ~eval-if~ there's no way to consider the evaluation of the predicate as linked to the argument.

This means our only option is to either use something other than and if statement or…

Well, we know how we're breaking things down, so in principle we can reverse things.
We can build an expresion in the object language via the meta-language.
So, the following is just longhand for ~lambda (x y z) (if x (y x) (expand-clauses z))~:

#+BEGIN_SRC racket
  (cons (make-lambda (list 'x 'y 'z) (make-if 'x ('y 'x) 'z)) (list (cond-predicate first) (cadr (cond-actions first)) (expand-clauses rest)))
#+END_SRC

So, we just branch to the above if some predicate ~cond-arrow-clause?~ returns true.
  ...
  (define (expand-clauses clauses)
	...
	(let ((first (car clauses))
	  ...
	  (if (cond-else-clause? first)
		  ...
		  (cons (make-lambda (list 'x 'y 'z)
				     (make-if 'x ('y 'x) 'z))
			(list (cond-predicate first) (cadr (cond-actions first)) (expand-clauses rest)))
...
Maybe this works?
I think the solution should be something along these lines, as the point of this procedure is to write object language expressions in the meta-langauge (now very obvious, but a little lost on me before…)
...
The idea is the expression is made up of two parts.
First, a list of variable and expression pairs, and second the body.

If we extract the variables, these can be passed to ~make-lambda~.
Then, we have the application of the lambda expression.

To get the list of variable expression pairs, then, we take ~cadr~ of the expression (as ~'let~ is ~car~).
And, using ~map car~ will get the list of all the variables.

Likewise, ~caddr~ gets us the body of the let expression, and hence the body of the lambda expression.

Though, to help keep things general and clear, we define a handful of helper procedures:

#+NAME: def:let-var-terms
#+BEGIN_SRC racket
  (define (let-var-terms exp)
    (map car (cadr exp)))
#+END_SRC

#+NAME: def:let-var-values
#+BEGIN_SRC racket
...
: 30
  ...
  With this, ~let->combination~ is simple:
...
(And earlier version used ~apply~ rather than ~cons~, but this isn't right.
For, at the moment our goal is only to rewrite expressions and ~apply~ makes stuff happen.
(It's not like ~make-if~. I.e. ~(apply x y)~ does not return ~(list 'apply x y)~.))
...
The second call to ~eval~ will then led to ~application?~ being called, etc.

** 4.7

Huh, the following works fine:
...
This can be used to tidy up some of the earlier procedures!
...
And, transformed into lambda's we'd get:
...
For a reduction, the idea is to recursively apply ~make-let~ until the vars have been exhasted:

#+NAME: def:make-let
#+BEGIN_SRC racket
  (define (make-let list-of-vars body)
    (append (list 'let list-of-vars) body))
#+END_SRC

Note, when making a let we can't assume the body is a single expression.
So, we need to append the body, rather than include it in the list.

#+NAME: def:let*->nested-lets
#+BEGIN_SRC racket
...
We don't want a let expression.
Rather, we want the body of a let expression, which is a sequence of expressions.
In our case, this is a single expression, but it's key to keep the distinction in mind.
...
#+BEGIN_SRC racket
  <<def:let*->nested-lets>>
  (define test (list 'let* (list (list 'a 1) (list 'b 2) (list 'c (list '* 'a 'b))) (list '* 'a 'b)))
  test
  (let*->nested-lets test)

  (define other-test '(let* ((x 3)
			     (y (+ x 2))
			     (z (+ x y 5)))
			(* x z)))
  (let*->nested-lets other-test)
#+END_SRC

#+RESULTS:
: (let* ((a 1) (b 2) (c (* a b))) (* a b))
: (let ((a 1)) (let ((b 2)) (let ((c (* a b))) (* a b))))
...
One worry is that eval is recursive.
Hence, it may appear the inner-most lambda is evaluated prior to any other.
However, this isn't clearly a problem.
For, with a compound procedure the process is evaluating the body of the procedure inside an environment extended by the parameters.
So, as long as the parameters have been evaluated, everything should be fine.

So, I think the question is whether we have any guarantee the environment has already been extended.
And, I think we do.
For, otherwise we have no guarantee of execution for any compound procedure.

~apply~ on a complex expression just evaluates the body of the procedure within an extended environment.
So, whether or not we need to explicity expand ~let*~ is going to depend on the way this is evaluated, no?

** 4.8

This is cool.
In short, this allows us to specify default values for variables.
This means we set up an environment where the terms of the bindings are fixed.
And, then define the procedure and execute the procedure on the default values.

In other words, we rewrite this:
...
To test which case we're in, we can see if the ~cadr~ of the ~let~ is a list.
If so, this is a standard ~let~ construction.
Otherwise, if it's a symbol, we have a named let construction.

Then, it's just about parsing everything and piecing it back together.

In particular, we create a ~let~ statement, a definition, and a procedure.

This is implemented in the evaluator.

** 4.9
*** while

The form is ~(while condition procedure)~.

Intuitively, this reduces to:

#+NAME: def:while-basic
#+BEGIN_SRC racket
...
However, we need to check the condition on each pass, so we need to pass in a procedure.
Likewise, we only want to evaluate the procedure when the condition is met, so this procedure should be wrapped in a procedure to.
This means a basic instance of while will involve a pair of lambdas (or something equivalent):

#+BEGIN_SRC racket
  <<def:while-basic>>
  (define x 1)
  (while (lambda () (< x 5)) (lambda () (display x) (newline) (set! x (+ x 1))))
#+END_SRC
...
We can do much better with a derived expression, as when evaluating ~while~ we can substitute a plain condition and body for the lambda wrapped versions.

For now, I think the observed derivation idea is sufficient.

This is implemented in the evaluator

** 4.10

As we're only implementing a new syntax, this amounts to updating the test conditions.
...
which would allow us to write ~(def …)~ rather than ~(define …)~.

Or, we could reverse the order of every expression, by calling reverse on the evaluation of every expression.

Some things require more care.
For example, changing to infix notation for binary operators would be nice.
But, this requires us to check each of the tests to ensure an infix expression would not be misclassified.
As things stand, I think it should be easy to insert prior to the application case, though.

I leave the details to some other time.

** 4.11

#+NAME: def:frame-pairs
#+BEGIN_SRC racket
...
I think I understand why this isn't ideal.

The problem is, updating the frame.
To do this, frame needs to be a pointer to something, so that in the local binding the value of ~frame~ doesn't change.
This rules out the empty frame being ~nil~.

Hence, if no variables and values are given, we need an list which contains nil, rather than an empty list.

Of course, it may be there are guarantees this won't happen.
Though, Ex. 4.13 suggests this isn't a given.

Anyway, some tests:

#+BEGIN_SRC racket
  <<def:frame-pairs>>
  (define test-frame-nil (make-frame (list ) (list )))
  test-frame-nil
  (frame-variables test-frame-nil)
  (add-binding-to-frame! 'x 'y test-frame-nil)
  test-frame-nil
  (add-binding-to-frame! 'x2 'y2 test-frame-nil)
  test-frame-nil
  (define test-frame (make-frame (list 'a 'b 'c) (list 1 2 3)))
...
A different approach would be use ~make-frame~ as a way to unfold a a message passing system.
Though, this isn't ideal given the arguments passed to ~make-frame~.
Ideally, message passing procedure wouldn't store anything other than the frame.

With ~make-unbound!~ from Ex. 4.13 this could be achieved.
Though, I think this is good for now.

** 4.12

We can see both ~set-variable-value!~ and ~lookup-variable-value~ as instances of performing some action relative to a globally (i.e. trace back through to the global env, if needed) bound term.

To keep things very general, the action is given three arguments: the current vars pointer, the current vals pointer, and the current env.
Redundant arguments are discarded as needed.

#+BEGIN_SRC racket
  (define (act-on-global-bound-val var action env)
    (define (env-loop env)
  ...
  (define (set-variable-value! var val env)
    (act-on-global-bound-val var (lambda (vars vals env) (set-car! vals val)) env))

    (define (lookup-variable-value var env)
      (act-on-global-bound-val var (lambda (vars vals env) (car vals)) env))
#+END_SRC

By contrast, ~define-variable!~ works on the local frame only and doesn't require a bound variable:

#+BEGIN_SRC racket
  (define (act-on-local-var var found-action not-found-action env)
    (let ((frame (first-frame env)))
      (define (scan vars vals)
	(cond ((null? vars)
	       (not-found-action vars vals frame))
	      ((eq? var (car vars))
	       (found-action vars vals frame))
	      (else (scan (cdr vars)
			  (cdr vals)))))
      (scan (frame-variables frame)
	    (frame-values frame))))
  ...
  (define (define-variable! var val env)
    (act-on-local-var var
		      (lambda (vars vals frame) (set-car! vals val))
		      (lambda (vars vals frame) (add-binding-to-frame! var val frame))
		      env))
#+END_SRC

Things could be generalised further.
Though, thing requres configuring whether or not to run the ~env-loop~ of ~act-on-global-bound-val~.

** 4.13

We bind variables locally, and so we should also unbind variables locally.

Though, really, there's no way to really justify the choice, as there's no mention of what the goal of this procedure is.
In some cases, a complete removal of the binding may be useful.
However, I susspect being able to access an overwritten variable might be nice.

We'll make use of ~act-on-local-var~ to implement this.
And, in particular, we'll make use of a general procedure which removes the 'current' (or relative first) element from a list.

The removal procedure:

#+NAME: def:remove-current-elem
#+BEGIN_SRC racket
  (define (remove-current-elem l)
    (cond
      ((null? l) #f)
      ((null? (cdr l)) #f)
      ((null? (cddr l)) (set-cdr! l nil))
...
#+BEGIN_SRC racket
  <<def:remove-current-elem>>
  (define test-list (list 1 2 3 4 5))
  (remove-current-elem test-list)
  test-list
  (remove-current-elem (cdr test-list))
  test-list
  (remove-current-elem (cddr test-list))
  test-list

  (define empty-list (list ))
  (remove-current-elem empty-list)
#+END_SRC
...
And, now ~make-unbound!~ via ~act-on-local-var~.
The not-found procedure doesn't do anything (though could return an error).
And, the found prcoedure removes the current var and val from the lists.

#+BEGIN_SRC racket
  (define (make-unbound! var env)
    (act-on-local-var var
		      (lambda (vars vals frame) (#f))
		      (lambda (vars vals frame) (begin
						  (remove-next vars)
						  (remove-next vals)
						  #t))
		      env))
...
~map~ applies a procedure to elements of a list.
...
We've evaluated everything in sub-environments of the meta-global-environment.

So, in particular, the evaluation of everything happens outside the metacircular evaluator.

Focus on proc.
This is evaluated by the metacircular evaluator.
As we have a definition, this leads to a compound procedure.

If the compound procedures differ between the evaluator and the metacircular evaluator, then map fails to apply the procedure to the arguments.

Testing things out, this seems to be the issue.

Though, I don't think this is a good exercise.
For, this relies on knowing the meta-language is distinct from the object language.
Specifically, in the handling of procedures.
And, I don't think there's any way to be sure of this without some experimentation.
...
The ~if~ condition checks to see whether the execution of the procedure ~try~ halts when given the procedure ~try~ as an argument.

Suppose ~(halts? try try)~ is true.
Then ~(try try)~ runs forever.
A contradiction.

Suppose ~(halts? try try)~ is false.
Then, ~(try try)~ returns ~'halted~.
A contradiction.

Constructing a universal turing machine within a language is enough to show the language is turing complete.
And, if you can construct a universal turing machine, then you have a way of treating procedures as data.
So, for any turing complete language, there's some way to consider procedures both as instructions and as data.

We haven't done too much with reading and writing data so far.
Though, with the discussion of environments it's clear to see we can construct a tape for a turing machine of arbitrary length by defining either variables or a list.
...
This is revised from a first attempt, which was very messy and not particularly good.

The key idea of the revised version is we have a recursive procedure which focuses on lambda expressions and appeals to the way definitions and applications are handled for the general case.

Specifically, in the case of definitions the proc returns the definition with the lambda expansion (if any) scanned-out and applications are scanned out instruction by instruction.

The scanning out starts by creating a term list and separating the body of the lmabda expression from the parameters.
The body is scanned, and when a definition is found this is scanned out, with ~scan-out-defines~ being recusively applied to the (lambda-expanded) body.

This only happens when a definition is scanned out, and if this is the case then the body of the lambda is a sequence, so we append the scanning out with ~begin~.

Then, a lambda is made with the scanned out body and empty arguments.

All of this is returned only if scannning out happens, else everything would be wrapped in lambdas.
...
#+BEGIN_SRC racket
  (define f (lambda (x) ((lambda (b a) (begin (set! a 4) (set! b 5) (+ a b x))) '? '?)))
#+END_SRC

***** more lambda, no defs

#+BEGIN_SRC racket
(scan-out-defines '(((lambda (x y)
                     (lambda (a b c d e)
                       ((lambda (y z) (* x y z))
                        (* a b x) (+ c d x))))
                   3 4)
...
#+BEGIN_SRC racket
  (scan-out-defines '(define (cube-root x)
  ...
  (define cube-root
    (lambda (x)
	 ...
	 (begin
	   (set! cube-iter
		 (lambda (guess)
		   ((lambda (improve good-guess? cube)
		      (begin
			(set! cube (lambda (x) (* x x x)))
			(set! good-guess? (lambda (guess) (< (abs (- (cube guess) x)) 0.001)))
			(set! improve (lambda (guess) (/ (+ (/ x (* guess guess)) (* 2 guess)) 3)))
			(if (good-guess? guess) guess (cube-iter (improve guess)))))
		    '?
		    '?
		    '?)))
	   (cube-iter 1.0)))
...
~make-procedure~ is only invoked when a lambda expression is evaluated.
Though, ~procedure-body~ is only evaluated given a compound-procedure.

So, at the core, the only non-primitve application is beta reduction.

In this respect, it seems preferable to include ~scan-out-defines~ in ~make-procedure~, as we won't then need to reapply the transformation every time the procedure is evaluated.

** 4.17

There's an extra frame as the ~let~ construction hides the evaluation of a lambda expression, where the terms of the let placed in the created frame.

Definitions, by contrast, simply add a term to the current frame.

This never makes a difference as the frame introduced by ~let~ is transparent with respect to everything but the terms defined.
Though, this could be a problem if there is checking for term collision.
Generally speaking, this is probably why procedures aren't easily included in let statements.

I'm not sure why we'd want to do this.
But, we could introduce a parallel of the lambda construction which writes directly to the current frame.

(Note, there's no need to make this public.)

** 4.18

So, u and v now point to terms of a sub-frame.
However, we make this assigment in the sub-frame, where we have access to u, v, and the terms of the sub-frame.

So, this should work so long as we're not evaluating anything.
...
#+BEGIN_SRC racket
  (let ((a 3))
    (let ((b 4))
      (set! a b)
      (display "inner a call: ")
      (display a)
      (newline))
    (display "outer a call: ")
    (display a)
    (newline))
#+END_SRC
...
However, with cross reference, there's an issue.
For example:
...
Here, we'd apply ~(+ a 1)~ as an argument to the lambda expression obtained by transforming the inner let.
But, ~a~ is not in the outer frame.

In the text a delay is involved:

#+BEGIN_SRC racket :tangle no
  (define (solve f yO dt)
    (define y (integral (delay dy)))
    (define dy (stream-map f y))
    y)
#+END_SRC

Here, we don't need the value of ~dy~ to evaluate ~y~.
However, what to make of the delay?
Plausibly, we still want a link to ~dy~ in order to know what to ~force~.
Though, I guess in principle ~delay~ could add a new variable to the frame, and then store the un-evaluated input.
Which, maybe, makes more sense, for a 'true' delay.
I mean, nothing about the environment should change in a way that matters (so long as we don't allow unassignment).

So, maybe this works…

** 4.19

Yeah, Eva's view makes sense.

The definitions are meant to be simultaneous, then when evaluating ~b~ the question is what is the value of ~a~ now?
And, right now ~a~ has the value 5, as at the same time as ~b~ is set, ~a~ is also set.

I guess you'd want to figure out a unique (maybe up to value assigned isomorphism) dag, and return an error if none exists.
...
As with other let transformations, it's easy to get a list of the terms.
And, this time the transformation is highly descrtuctive.
First, we change all the assignemnts to set.
Second, as these are now procedures to execute, we merge the assignments with the body of the letrec expression.

This is implemnted in the evaluator, with the key procedures reproduced here:
...
Note the double quotation of ~*undefined*~.
I'll figure out why this is the case some other time…
In contrast to the use of ~*unassigned*~ in ~scan-out-defines~, we're evaluating each ~*unassigned*~, given these are passed as arguments to a lambda term.
Though, I'm not sure why this would lead to the term being understood as a variable…

To be honest, I think a non-destructive approach would have been easier.
Only need to map the terms and varibles to a set! expression and then append the body.
Ah well…

*** b.

This is highlighting how let statements involve evaluation due to really being lambda expressions, no?
I figure this should be clear by now…
...
Following this pattern, the Fibonacci numbers is straightforward:
...
This is a fixed point combinator, right?

Mostly.

It's quite nice.

After setup we have ~(fb lambda.fb n)~.
So, the first instance of ~fb~ is evaluated fully.
If ~k~ is not a base case, then this results in repeat instances, with a different ~k~.

Right, this is just a fixed point combinator, where a base case is included.
Ignoring the base case, ~lambda.fb~ is of the form ~(lambda (x) (f x x))~.
...
Right, and a fixed points of two mutually defined functions.
Neat.

** 4.22

There's very little to do here, as let (and all the variations of let considered) are derived.

So, for something a little more involved, ~while~ is also updated.

Here, the idea is to save both the analysis of the predicate and body, then create an internal if-then-loop which only works with the evaluated expressions.
As, looping on ~analyze-while~ would require re-analysis every time.
...
Note, it would be nice to extend this to allow specifying a post condition.
Another extension would be to specify local variables with initial values.
...
This is the same issue as ~while~.
If procedures are internal to ~execute-sequence~, then these are analysed on every recursive call.
Hence, we need an internal loop procedure.

** 4.24

Some other time!

At the moment, all interaction with the evaluator is direct.
To do some tests, I'd like some way to automate everything.
...
If ~unless~ simply passed the arguments through to ~if~, then things would be fine.

Still, as ~unless~ is a definition, both arguments are evaluated in applicative-order.
The condition is easy, as it's just a predicate.
The usual value is a problem, as this triggers further calls to factorial.
In particalar, we'll escape the base case when ~n~ is 1.
And, there's nothing that can be done to avoid this.
Even if the condition is triggered for any n < 2, it will still be evaluated for each n.

In normal order, arguments are evaluated as needed, and so ~unless~ becomes transparent to the underlying ~if~.

To help see this, evaluate the following syntactic rewrite:
...
The observation that ~unless~ may be implemented as a special form was made in the previous exercise.

I can't think of any case in which you'd want both ~unless~ and ~if~.
...
When something is defined, the value of the definition is evaluated.

In the case of ~id~ we have a lambda expression.
So, a procedure is created.

In the case of ~w~, we have ~(id (id 10))~.
This is an application, so the value is forced.
I.e., ~count~ is incremented by 1, as ~x~ is considered.
In this case, ~x~ is delayed ~(id 10)~, as an argument is delayed unless forced.

So, ~count~ is ~1~.

Further, as everything is memoised, the value of ~w~ is nw the delayed ~(id 10)~.

So, when ~w~ is called, ~(id 10)~ is evalauted, which increments ~count~ by 1 and returns ~x~.
In this case, ~x~ is ~10~, and ~10~ is primitive, so ~10~ is returned.
And, given memoisation, the value of ~w~ is now ~10~.

Without memoisation, the value of ~w~ is fixed at ~(int 10)~, so ~count~ is incremeneted with every call of ~w~.


I guess definitions involve evaluation as we're thinking of these as passing an argument to a procedure, where the procedure then does some work.

The issue I have is the status of ~if~.
There's nothing to distinguish the behaviour of ~if~ from any other procedure, but evaluation is selective.
And, it seems reasonable to treat ~define~ similarly.

(Indeed, this works out, and ~count~ is incremented twice with ever call of ~w~
Having an evaluator is very nice!)
...
Without ~actual-value~, what would be ~+~ is a thunk, and the evaluation fails.
With ~actual-value~ the ~a~ arg is evaluated as ~+~ and everything follows through.

** 4.29

I'd expect any procedure which reapeatedly takes the same complex argument to do worse.

For example, a sequence of conditions testing whether some argument sstisfies one of many predicates:
...
There's no need to force the thunk every time the argument is tested.

When the evaluator memoises, the count only ever reaches 2.
Otherwise, ~(id 10)~ is called every time ~w~ is called, and the count may be increased arbitrarily.

** 4.30

*** a.

In the evaluator ~eval-sequence~ reads:
...
So, ~eval~ is appled to each element in the sequence.

There's no real surprise ~for-each~ given by:
...
Handles things correctly when ~proc~ is a lambda expression, as the expression is evaluated and applied to each of the items.
...
~p1~ updates the value of the argument term, and returns the vale of the term.

~p2~ calls ~p~ which performs some expression and then returns the value of the term.
Here, the result is as Cy's predicted.
However, it's not due to sequences.
Rather, it's due to ~set!~ being passed as the argument of a procedure.
For example, the following procuces a pair:

#+BEGIN_SRC racket
  (define (p2 x)
    (define (p e) (set! x (cons x '(2))) x)
    (p #f))
#+END_SRC

The issue is, everything that is passed as an argument needs to be either a primitive or forced to be evaluated.
Otherwise, apply just looks up the think associated with the term representing the argument.

With the proposed changes, ~p2~ produces a pair, in particular, because the value of the term ~e~ is forced.

*** c.

Why would it?
~force-it~ is transparent unless the object to which it is applied is either a thunk or evaluated thunk.

*** d.

So, one possible change is to ~lookup-variable-value~.
Here, we'd check to see if any variable (i.e. term) we'e trying to access is a thunk, and force the evaluation if it is.
...
With this change, ~p2~ produces a pair.

** TODO 4.31

There are at least two ways to go here.

First, we could keep lazy evaluation as the fundamental menthod, but default to forcing terms.

I think this is a minor change, as it just amounts to a check on ~lookup-variable-value~.
At the moment we have can add in ~force-it~ which is memoised, but it's easy to add an additional option.

Second, we could extend a applicative order base.
A kind of easy way to do this is with lambdas.
Whenever ~lazy~ appended to an argument, the argument is wrapped in a lambda expression.
And, whenever ~lazy-memo~ is appended to an argument, the argument is wrapped in a memod lambda expression.

The latter seems nice, but I'll leave this for a rainy day.

** Non-strict primitives

Implementing lazy cons, etc. is a headache.

This is because it's natural to define the lazy version of  ~cons~ in the base language as a distinct procedure, and then treat this as a primitive procedure.

However, even though most evaluation is lazy, for primitive procedures applicative evaluation is performed.
So, the option takes is to implement semi-primitive-procedures.
These rely on base ~apply~.
However, the arguments given are still delayed.

The only issue here is that some manual forcing is requred in the case of ~car~ and ~cdr~.
For, these take a delayed cons as an argumnet.
Indeed, for any procedure of this kind, some explicit forcing is requred.

Fair, the text reads:

"One way to accomplish this is to extend the lazy evaluator to allow for non-strict primitives…"

So, this issue is explicit.
But, this wasn't focused on enough to really register with me.

In any case, the evaluator now supports non-strict primitives via 'semi-primitive' operations.
These work the same way as primitives, and are added to the enviroment by extending the list of primites.
...
So, a headache, but worthwhile understanding.
Lazy evaluation is really nice in some respects, but the various ways in which things can be lazy makes for trouble.

With applicativ evalation, ~if~, ~defines~, and so on are non-standard, which can be a little difficult at times.
But, with normal/lazy evaluation it feels as though almost every procedure /may/ be non-standard, as everything depends on the way forcing is handled.

Also, suggesting a redefinition of ~cons~, etc. seems like a terrible idea.
For, this amounts to modifying the meta-language with the object-language.
Of course, it works out, but I can't see this being good practice.

** 4.32

We can now easily create a stream of stream, (i.e. streams indexed by a stream).

Though, I don't really see anything of interest other than this.
For, the only difference is that lazy evaluation applies to the ~car~ of a ~cons~.
Anything that can be done without this was possible without the extra laziness.

** 4.33

Following the non-strict primitive approach, lists are defined by:
...
And, ~text-of-quotation~ is updated to:
...
There's no need to apply quotation to the elements of the quote when doing this, as at present, the elements are uninterpreted.

** 4.34

This exercise… is something I'll return to.

In retrospect, it makes far more sense to implement lazy pairs directly in the eval loop.
We'd then have somethign tagged with 'cons' which would allow modifying user-print easily.

I don't think there's a good general solution given streams are possible.
An earlier exercise sketched out how to check for loops in a list of lists, and this could be used to display the list/stream up to some depth.

Alternatively, the generating description of the list of could be saved.
This would take a lot of overhead, but would return a way to understand the object.
Indeed, the same understanding used to generate the list.
Though, this would only apply when displaying the initial list.

Or, do the scheme thing, which is to print something like 'promise'.

With lazy evaluation this is also a headache.
As, if evaluation has side effects, then it's unclear whether these should result from display.
Should the element be evaluated, or should the thunk be shown?

** 4.35

#+NAME: def:amb-an-integer-between
#+BEGIN_SRC racket
...
Note, additional calls to ~amb~ produce the next alternative.
This is sure to be explained later…

** 4.36

We have the following:

#+NAME: def:amb-require
#+BEGIN_SRC racket
  (define (require p)
    (if (not p) (amb)))
#+END_SRC

#+NAME: def:amb-a-pythagorean-triple-between
#+BEGIN_SRC racket
...
Replacing ~(an-integer-between a b)~ by ~(an-integer-starting-from a)~ wouldn't work as no limit would be placed on ~k~.
Hence, as ~amb~ is depth first search, additional values of ~k~ would be attempted, and no other values of ~i~ and ~j~ would be considered.

As we're not asked for a procedure which outputs triples uniquely, we can in principle generate all triples by repeatedly generating triples between 1 and some finite limit and then increasing the limit when needed:

#+BEGIN_SRC racket
  <<def:amb-a-pythagorean-triple-between>>
  (define (all-triples)
    (define (all-triples-with-lim i)
      (amb (a-pythagorean-triple-between 1 i) (all-triples-with-lim (+ i 1))))
    (all-triples-with-lim 1))
...
This method looks to be faster in general.
For, with the previous method, each k ≤ j would be explored.
And, with the current method, this no longer happens.
...
There are five solutions to the modified puzzle!
...
The goal is to structure the requirements so that redundant options are not explored, or explored fewer times than otherwise.

This can certainly make a difference.
For example, if ~distinct?~ was the last constraint, all the other requirements may pass when everyone is assigned ~4~.

As a heuristic, we'd like to order things most choice to least choice.
This way, when a new alternative is requested, there are fewer requirements to backtrack and forwardtrack through.

This could for sure be solved properly by working out the actual number of possible combinations considered.
...
#+RESULTS: def:an-element-of

And custom.

~l-without-e~ returns a list without the elements which satisfy some procedure.

Note, this is just ~filter~.
Completely overlooked this when making the proc.

#+NAME: def:l-without-e
#+BEGIN_SRC racket
  (define (l-without-e proc l)
	(cond ((null? l)
	       '())
	      ((proc (car l))
	       (l-without-e proc (cdr l)))
	      (else
	       (cons (car l) (l-without-e proc (cdr l))))))
#+END_SRC

~adjactent-to?~ is self explanatory.

#+NAME: def:adjactent-to?
#+BEGIN_SRC racket
  (define (adjactent-to? i)
    (lambda (x) (= (abs (- x i)) 1)))
#+END_SRC

And, ~ints-between~ makes a list of the integers between ~a~ and ~b~.

#+NAME: def:ints-between
#+BEGIN_SRC racket
  (define (ints-between a b)
    (if (> a b)
	nil
	(cons a (ints-between (+ a 1) b))))
#+END_SRC

With these, we can eliminate most of the requirements be restricting the possible dwellings, either at the initial stage or by a combination of the given procedures.
...
#+BEGIN_SRC racket
  (define (multiple-dwelling)
    ...
    (let ((baker (amb 1 2 3 4))
	  (fletcher (amb 2 3 4))
	  (miller (amb 1 2 3 4 5)))
      (let ((cooper (an-element-of (l-without-e (adjactent-to? fletcher) (ints-between 2 miller))))
	    (smith (an-element-of (l-without-e (adjactent-to? fletcher) (ints-between 1 5)))))
	;; (require
	;;  (not (= (abs (- smith fletcher)) 1)))
	(require
	 (distinct? (list baker cooper fletcher
...
But this is not too interesting.

The only requirement left is that the dwellings are distinct.
And, we only need to fill in three distinct values.

So, basically, we just write a procedure which fills in possible values for ~baker~, ~fletcher~, and ~miller~.
For ~cooper~ and ~smith~ we then check the lists produced to see if there are any distinct pairs.
If so, each of these is a solution.
Otherwise, no solution was found with the inputs.

That's 30 possible combinations to do by hand, but a procedure should be easy to find.

I don't think writing the proceudre is particularly interesting.
So, this is where I'll leave things.

** 4.42

We make use of exclusive or to model the statements:
    ...
    (let ((betty (amb 1 2 3 4 5))
      ...
      (require
       (distinct? (list betty ethel joan
			kitty mary)))
      (require (x-or (= kitty 2) (= betty 3)))
      (require (x-or (= ethel 1) (= joan 2)))
      (require (x-or (= ethel 5) (= joan 3)))
      (require (x-or (= kitty 2) (= mary 4)))
      (require (x-or (= mary 4) (= betty 1)))
...
I was thinking about skipping this, but it turned out to be a lot of fun.
What convinced me to pursue was the obervation that this is solved using ~amb~ with respect to relations, rather than assignments.

Starting with the key idea, ~amb-pair~ makes an ambinvalent relation by ambinvalently taking elements from two lists.

#+BEGIN_SRC racket
  (define (amb-pair l1 l2)
    <<def:an-element-of>>
    (cons (an-element-of l1) (an-element-of l2)))
#+END_SRC

However, to solve the problem we only want ambivalence in the right relata.
So, this is more of an ambiguous function.

#+NAME: def:amb-pair-r
#+BEGIN_SRC racket
  (define (amb-pair-r e l)
    <<def:an-element-of>>
    (cons e (an-element-of l)))
#+END_SRC

We now have a few procedures for inspecting lists of pairs.
The only slight headache here was remembering to 'a' the ~car~ and ~cdr~ calls (i.e. to ~caar~ and ~cdar~.)

#+NAME: def:matches
#+BEGIN_SRC racket
  (define (matches e v l)
    (cond ((null? l)
	   #f)
	  ((and (equal? e (caar l)) (equal? v (cdar l)))
	   #t)
	  (else  ;(not (equal? e (car l)))
	   (matches e v (cdr l)))))
#+END_SRC
...
#+NAME: def:reverse-lookup
#+BEGIN_SRC racket
  (define (reverse-lookup e l)
    (cond ((null? l)
	   #f)
	  ((equal? e (cdar l))
	   (caar l))
	  (else  ;(not (equal? e (car l)))
	   (reverse-lookup e (cdr l)))))
#+END_SRC
...
#+BEGIN_SRC racket
  (define (boats-and-daughters)
    ...
    (let* ((daughters '(lorna melissa rosalind gabriella mary))
	   (fathers '(moore downing hall hood parker))
	   (farther-relation (map (lambda (x) (amb-pair-r x daughters)) fathers))
	   (boat-relation (map (lambda (x) (amb-pair-r x daughters)) fathers)))
      (require (distinct? (map cdr farther-relation)))
      (require (distinct? (map cdr boat-relation)))
      (require
       (accumulate (lambda (x y) (and x y)) true (map (lambda (x y) (not (equal? x y))) farther-relation boat-relation)))
      (require (matches 'hood 'gabriella boat-relation))
      (require (matches 'moore 'lorna boat-relation))
      (require (matches 'hall 'rosalind boat-relation))
      (require (matches 'downing 'melissa boat-relation))
      (require (matches 'hood 'melissa farther-relation))
      ; (require (matches 'moore 'mary farther-relation))
      (require
       (equal?
	(lookup (reverse-lookup 'gabriella farther-relation) boat-relation)
	(lookup 'parker farther-relation) ))
      (display (reverse-lookup 'lorna farther-relation))))

  <<def:amb-exhaust>>
  (exhaust boats-and-daughters)
#+END_SRC

Running without the comment, we find lorna's father is downing.
With the constraint commended, parker is also an option.

Whether or not this is (relatively) efficient I'm not sure, but it's cool to see ambinvalent relations at work!

** 4.44

To start, a pair of somewhat messy procedures to produce a list of pairs corresponding to other elements in the diagonal of some element.

#+NAME: def:diagonal-down
#+BEGIN_SRC racket
  (define (diagonal-down pair to)
    (define (back-to-start pair)
      (if (or (= 1 (car pair)) (= 1 (cdr pair)))
	  pair
	  (back-to-start (cons (- (car pair) 1) (- (cdr pair) 1)))))
    (define (diag-from-except pair ex-pair to)
      (cond ((or (eq? (+ to 1) (car pair)) (eq? (+ to 1) (cdr pair)))
	     nil)
	    ((or (eq? (car ex-pair) (car pair)) (eq? (cdr ex-pair) (cdr pair)))
	     (diag-from-except (cons (+ (car pair) 1) (+ (cdr pair) 1)) ex-pair to))
	    (else (cons pair (diag-from-except (cons (+ (car pair) 1) (+ (cdr pair) 1)) ex-pair to)))))
    (diag-from-except (back-to-start pair) pair to))
#+END_SRC


#+NAME: def:diagonal-up
#+BEGIN_SRC racket
  (define (diagonal-up pair to)
    (define (back-to-start pair)
      (if (or (= to (car pair)) (= 1 (cdr pair)))
	  pair
	  (back-to-start (cons (+ (car pair) 1) (- (cdr pair) 1)))))
    (define (diag-from-except pair ex-pair to)
      (cond ((or (eq? 0 (car pair)) (eq? (- to 1) (cdr pair)))
	     nil)
	    ((or (eq? (car ex-pair) (car pair)) (eq? (cdr ex-pair) (cdr pair)))
	     (diag-from-except (cons (- (car pair) 1) (+ (cdr pair) 1)) ex-pair to))
	    (else (cons pair (diag-from-except (cons (- (car pair) 1) (+ (cdr pair) 1)) ex-pair to)))))
...
Second, a way to generate rows:

#+NAME: def:row
#+BEGIN_SRC racket
      (define (row i from to)
      (define (ints from to)
	(if (> from to)
	    nil
	    (cons from (ints (+ from 1) to))))
      (map (lambda (x) (cons x i)) (ints from to)))
#+END_SRC

(I was too lazy to find the equiv of ~ints~ surely defined above somewhere.)

With this, ~queens~ amounts to specifying a queen for each row, and then requiring the rows, cols, and diagonals are distinct.
...
#+BEGIN_SRC racket
  (define (queens)
    ...
    <<def:diagonal-down>>
    <<def:flatmap>>
    (let ((q1 (an-element-of (row 1 1 8)))
	  (q2 (an-element-of (row 2 1 8)))
	  (q3 (an-element-of (row 3 1 8)))
	  (q4 (an-element-of (row 4 1 8)))
	  (q5 (an-element-of (row 5 1 8)))
	  (q6 (an-element-of (row 6 1 8)))
	  (q7 (an-element-of (row 7 1 8)))
	  (q8 (an-element-of (row 8 1 8))))
	(require
	 (distinct? (map car (list q1 q2 q3 q4 q5 q6 q7 q8))))
	(require
	 (distinct? (map cdr (list q1 q2 q3 q4 q5 q6 q7 q8))))
	(require
	 (distinct? (flatmap (lambda (x) (diagonal-up x 8)) (list q1 q2 q3 q4 q5 q6 q7 q8))))
	(require
	 (distinct? (flatmap (lambda (x) (diagonal-down x 8)) (list q1 q2 q3 q4 q5 q6 q7 q8))))
	(list (list 'q1 q1)
	      (list 'q2 q2)
	      (list 'q3 q3)
	      (list 'q4 q4)
...
This isn't particularly effective, but I don't think that was the expectation of the exercise.
...
#+RESULTS:
: ((q1 (1 . 1)) (q2 (5 . 2)) (q3 (8 . 3)) (q4 (6 . 4)) (q5 (3 . 5)) (q6 (7 . 6)) (q7 (2 . 7)) (q8 (4 . 8)))

** Parsing natural language

#+NAME: def:nat-lang-parser
#+BEGIN_SRC racket
    <<def:amb-require>>
    (define *unparsed*
      '())
...
#+RESULTS: def:nat-lang-parser

** 4.44

I don't think there's a way to get the parser working with calls to ~amb~.
Though I'm not sure why.

Intuitively, a call to ~amb~ should make another call to ~parse~.
But, if it's really a call to ~require~ within ~parse~, then there is nothing left to do.

And, this seems like a branch point.
Looking ahead, the evalautor considers ~try-again~ as relative to the last procedure given as input.
So, this is the call to parse.
But contrast, ~amb~ may work with respect to the last procedure called.
In which case, it probably is the call to ~require~ on what is now a null term.
...
: amb tree exhausted
:   context...:
:    /var/folders/jg/7666sp2944d0tzp1vhtlf5pm0000gn/T/babel-lAsQCE/org-babel-V0ptBp.rkt:48:2: parse-sentence
:    /var/folders/jg/7666sp2944d0tzp1vhtlf5pm0000gn/T/babel-lAsQCE/org-babel-V0ptBp.rkt:63:0: parse
:    body of "/var/folders/jg/7666sp2944d0tzp1vhtlf5pm0000gn/T/babel-lAsQCE/org-babel-V0ptBp.rkt"

** Implementing the ~amb~ evaluator

The cycle of ~eval~ and ~apply~ extends the evaluator with analysis.

The evaluator is given instructions.
~eval~ is applied to these instructions


to figure out what they amount to in the meta-langauge and applies these to some given ~env~
In order to apply what is evaluated, we need to supply an ~env~, a ~success~ continutation, and a ~fail~ continuation.

Here, these need to be supplied as each requires the context of what has been applied so far as the role of ~eval~ is to figure out what to do regardless of context.
...
For every analysed argument, a lambda expression is returned with takes ~env~, ~success~, and ~failure~.

Key to keep in mind is that the success condition is always applied.

The important idea with everything here is that the analysed expression amounts to passing something forward.

Think about the ~if~ analysis.
The body of this analysis amounts to executing the condition, and then setting up what happens on success.
When the ~if~ is put into action, the condition is evaluated, and then applied to the success condition.
This isn't super clear /inside/ the ~if~ statement, but consider walking through a basic case like:
...
The analysed expression says, roughly, do the predicate, where success amounts to branching on the truth of the predicate, and failure amounts to whatever failure amounted to before.

This is the key thing to keep in mind.
Analysis doesn't return a direct instruction.
Rather, analysis returns some tubing.

And, in retrospect, it's clear this was also the case with basic analysis.
At no point did analysis do anything, it just set things up so that analysis only needed to happen once.

~ambeval~ is only called in the driver loop.

Of course, when things are set into motion only the next piece of tubing is consdiered, but that's all that's needed.

Ok, this is super cool.
There's not really any backtracking.
Rather, there's a branch, and things are set up so that in some cases, if you take the 'failure' branch, this ends up reverting or changing some things.

This would have been super helpful to have explicitly pointed out.

Something like:

"
Of course, backtracking isn't really possible.
Time marches on.
However, it's possible to undo changes.
And, if we keep track of a a way to undo any change, this works the same as backtracking.
"

A different option would have been to first introduce this kind of evaluation without modelling backtracking.
For example, but introducing detailed error messages about what happened.

Ah well, it makes sense after a while.
...
In the class the prof lectures with the cat to the student.
...
In the class with the cat the prof lectures to the student.
...
#+BEGIN_SRC racket
  (sentence
   ...
   (verb-phrase
    (verb-phrase
     (verb lectures)
     (prep-phrase (prep to) (noun-phrase (simple-noun-phrase (article the) (noun student)) (prep-phrase (prep in) (simple-noun-phrase (article the) (noun class))))))
    (prep-phrase (prep with) (simple-noun-phrase (article the) (noun cat)))))
#+END_SRC

The prof with the cat lectures to the student in the class.
...
The prof lectures to the student in the class who has a cat.
...
The prof lectures to the student who is in the class with the cat.

** 4.46

The relevant operands are parts of a sentence.
And, the parser is written with the assumption that ~*unparsed*~ contains the unparsed text as given.
However, if the operands are not evaluated left to right, there is no guarantee ~*unparsed*~ contains the unparsed text as given.
...
This allows for a simple loop.
For, we may call ~(parse-verb-phrase)~ after calling ~(parse-verb-phrase)~, and so on.
...
The idea was to ambiguously choose a word.
...
#+BEGIN_SRC racket
  (define (parse-word word-list)
    ...
    ;(cdr word-list)))
    (set! *unparsed* (cdr *unparsed*))
    (an-element-of word-list))
#+END_SRC

This generates the strucutre, and with enough tries starts to fill in the structure.

More could be done here, but testing painful, and, unfortunately, I don't find this sufficiently interesting to endure it.

** 4.50

To implement ~ramb~ naively, we create a procedure to shuffle a list.

With this, ~ramb~ is a simple edit to ~amb~ such that the initial choices are shuffled.

Suffling a list is little more than creating a list by taking elements of the initial list according to a randomly generated index.

#+NAME: def:shuffle-list
#+BEGIN_SRC racket
...
#+BEGIN_SRC racket
  <<def:shuffle-list>>
  (shuffle-list (list 1 2 3 4 5))
#+END_SRC

#+RESULTS:
: (2 5 3 1 4)

My guess is this allows for a variety of sentence stuctures to be generated.
...
Which is just ~analyze-assignment~ without a distinct 'course of failure' for the assignmnet.
Alternatively, without an alternative path for the assignment.

~eq?~ also added a primitive to the evaluator for this.
...
When executing, we skip a display for when x = y, but ~count~ continues to go up.
There are nine assignmnets total.
The final value of count is 8 as we could how many times… alternative values for x and y are chosen.

If we'd have used set, count would always be 1.

** 4.52

~if-fail~ is analysed by:
...
When called, the main procedure is evaluated and called.
The alternative to applying the main condition is whatever the prior failure condition was.
When the main procedure is evaluated, though, the alternative condition is now the specific fail condition.
And, the success condition is apply the return value of the main procedure to whatever the given success was.

Note: the alternative procedure is only analysed if required.
My reading here is this is usually used to terminate the current computation.
So, there's no reason to store the analysis.
Of course, it's easy to keep a store of the analysis along with the main procedure if desired.
...
And, of course, passing procedures is possible…
...
It should be the case ~pairs~ repeated works through the supplied pairs to find pairs such that sum is a prime number.
When no more pairs are found, the list is returned.

This is what happens.

Below is background definitions for entering into the evalautor.
...
Note, if the predicate fails, the goal of ~require~ to only to explore a different path.
And, that alternative path is supplied by the procedure which accesses the value.
...
#+BEGIN_SRC racket
  (and (salary (Bitdiddle Ben) ?amount-b)
	      (salary ?x ?amount-x)
	      (lisp-value < ?amount-x ?amount-b))
#+END_SRC
...
#+BEGIN_SRC racket
  (assert! (rule (same-job ?p1 ?p2)
		 (and (and (job ?p1 ?j)  (job ?p2 ?j))
		      (not (same ?p1 ?p2)))))

  (assert! (rule (can-do-job ?p1 ?p2)
		 (and (and (job ?p1 ?j)  (can-do-job ?p2 ?j))
		      (not (same ?p1 ?p2)))))
...
Then, ~(replace ?p1 (Fect Cy D))~ returns:
...
With the rules from part 1, query:
...
To build the ~big shot~ query, we first build a ~supervisor-in-division~ query.

This finds ~p1~ and ~p2~ such that ~p1~ has a job in division ~d1~ and ~p2~ has a job in division ~d2~.

Importantly, this is a transitive relation.
(Note, four arguments are key to making this work (in a simple way, at least)!)

~supervisor-in-division~ can then be used with the same division given as an argument.
And, with this ~big-shot~ is easy.

#+BEGIN_SRC racket
  (assert! (rule (supervisor-in-division ?p1 ?p2 ?d1 ?d2)
		 (and
		  (job ?p1 (?d1 . ?sub-d))
		  (or
		   (and (job ?p2 (?d2 . ?sub-d-alt))
			(supervisor ?p1 ?p2))
		   (and
		    (supervisor ?p1 ?p3)
		    (supervisor-in-division ?p3 ?p2 ?d3 ?d2))))))

  (assert! (rule (big-shot ?p ?d)
		 (and
		  (job ?p (?d . ?sub-d))
		  (not (supervisor-in-division ?p ?p2 ?d ?d)))))
#+END_SRC
...
I'm not sure there is a natural way to avoid repetition.
The thing is, this is a symmetric relation.

So, in order to avoid both instances of the relation, we'd need to take advantage of the order by which queries are processed.
This might be possible, but it's not clear this is intended.
...
The relation is left-right only, so this is everything.
...
#+BEGIN_SRC racket
  ...
  (assert! (rule (last-pair (?z . ?t) ?x)
		 (last-pair ?t ?x)))
#+END_SRC

Seems to work for ~(last-pair ?x (3))~, in that the process does not terminate.

This terminolgy makes no sense though.
For sure, this is a representation of the last pair of a lisp list, given the cdr is nil.
But, we're not working with lisp lists, at least not by design…

Getting the last /element/ is basically the same, and more intuitive.

#+BEGIN_SRC racket
  (assert! (rule (last-element (?x) ?x)))
  (assert! (rule (last-element (?z . ?t) ?x)
		 (last-element ?t ?x)))
#+END_SRC

** 4.63

I guess the general son-of relation isn't ideal:

#+BEGIN_SRC racket
  (assert! (rule (son-of ?a ?b)
		 (or
		  (son ?a ?b)
		  (and (son-of ?a ?c)
		       (son-of ?c ?b)))))
...
As, there's no way to keep track of which son-of instances that have been explored.

Grandson is simple.

#+BEGIN_SRC racket
  (assert! (rule (grandson ?a ?b)
		 (and (son ?a ?c)
		      (son ?c ?b))))
#+END_SRC

My guess is this exercise was designed to push toward attempting to take the transitive closure and realising a problem…
...
An attempt to solve ~(outranked-by ?staff-person ?boss)~ may lead to an attempt to solve ~(outranked-by ?middle-manager ?boss)~ which is the same problem.

** 4.65

There are multiple middle-managers, and a result is returned every time a distinct collection of variables satisfies the rule.

** 4.66

The possiblility of duplicate results, I'd guess.

I'd probably require the specification of an identifier, and then temporarily store the results via the identifier.
One could then overwrite without checking, or perhaps call an error if an existing identifier is given a novel value.

In any case, some way to identify duplicate results is required, and in general this is going to require a store of all the (unique) seen results.

** 4.67

The goal is to ensure we do not instantiate a variable in a loopy way.

Doing this with variables alone seems impossible.
FOr, we have no way to tell from variables alone whether or not the data is well-grounded.

So, store a history of vairables instantiated, and ensure no attempt is made to substitute the same value for the same variable.
...
#+BEGIN_SRC racket
  ...
  (assert! (rule (reverse (?x . ?z) ?y)
		 (and (append-to-form ?zr (?x) ?y)
		      (reverse ?z ?zr))))
#+END_SRC

** TODO 4.69

When I have the motivation… something about writing queries in this system is very demotivating…
...
Introduces a loop, as the evaluation of ~THE-ASSERTIONS~ is delayed within ~cons-stream~ but not with respect to ~set!~.

So, that is, when ~(stream-cdr THE-ASSERTIONS)~ is evaluated, this is just whatever ~THE-ASSERTIONS~ has been set to, which in this case would be ~(cons-stream assertion THE-ASSERTIONS)~.

The let statements store a distinct reference to ~THE-ASSERTIONS~ so that this sort of loop can be avoided.

** 4.71

~stream-append-delayed~ works the same as ~stream-append~, with the expection of never evaluating the delayed argument.
And, as we're working with streams, the only time the argument is forced is when the appended element is accessed.

Using the alternative ~stream-append~ would involve evaluating the argument, and then appending the evaluated argument to the stream.

With ~simple-query~, rules are evalauted only after assertions are found.

So, one issue may be a rule which extends an assertion.
E.g., something where a base case exists, but won't be found as it's been entered as an assertion, rather than as an instance of the rule.

Maybe?

** 4.72

If ~disjoin~ didn't interleave streams, the stream for the first disjunct would be created before the second, and so on.
This is an issue if the data base is infinite.

Further, it's also likely to be ineffective, even in finite streams.
As, a disjunction is likely used because each of the disjuncts is satisfied by some entry.
So, working through one disjunct at a time is likely a block on making progress with various other disjuncts.

~flatten-stream~ takes a stream of streams and flattens these into a single stream.
Again, if the inner streams are infinite, the elements of any steams scheduled to be processed after the stream will never be seen.

Here, it's less clear how to motivate efficiency, as the process is higher order.
...
~flatten-stream~ recursively calls ~flatten-stream~.
So, in the call to ~interleave~, and additional call to ~flatten-stream~ is made.
In turn, this makes a call to ~interleave~, and the entire stream ends up being inspected.
So, we're not really using streams.

This is quite bad in the case of infinite streams.

** TODO The remainder of chapter 4

I'll perhaps return to these when I'm more enthusiastic about the query system…
...
Though, there should also be 1's going to counter and product.

The controller, text form:
...
Note, the additional 1's are important to describe a(n iterative) factorial machine.
Else, we just have a description of the inner ~iter~ machine.
But, these are discarded after first use, so they don't enter into the control loop.

One question is, if we're doing things sequentially, is it possible to reduce s and t to a single register?
If the controller could specify when an operation happens, then for sure.
We could update the product and then update the counter.
...
#+BEGIN_SRC racket
  (data-paths
   (registers
    ((name counter)
     (buttons ((name counter<-s) (source (register s)))))
    ((name product)
     (buttons ((name product<-t) (source (register t)))))
    ((name s)
     (buttons ((name count-up) (source (operation add1)))))
    ((name t)
     (buttons ((name new-product) (source (operation up-prod)))))
    )
   ...
   (operations
    ...
    ((name >)
     (inputs (register n) (register counter)))
    )
...
#+BEGIN_SRC racket
  (controller
   ...
   (branch (label fact-done))
   (assign s (op +) (reg counter) (const 1))
   (assign t (op *) (reg counter) (reg product))
   (assign counter (reg s))
...
Ah, with this, it's easy to reuse ~s~ (or ~t~)…
...
#+BEGIN_SRC racket
  (controller
   ...
   (test (op >) (reg counter) (reg n))
   (branch (label fact-done))
   (assign s (op *) (reg counter) (reg product))
   (assign product (reg s))
   (assign s (op +) (reg counter) (const 1))
   (assign counter (reg s))
   (goto (label test-n))
   fact-done
...
Note, we now need to ensure the next product has been calculated before increasing the counter.
...
As drawing isn't too easy, we just write out the short-form controller for calculating square-roots by the newtonian method.
...
So, first pass, treating ~good-enough?~ and ~improve~ as primitive:
...
#+BEGIN_SRC racket
  (controller
   ...
   (assign new-guess (op improve) (reg guess))
   (assign guess (reg new-guess))
   (goto (label test-guess))
   sqrt-done
   )
#+END_SRC

We expand on ~improve~ with an additional variable to handle taking the average of ~guess~ and ~(/ x guess)~.
Both ~+~ and ~/~ are assumed primitive.
...
#+BEGIN_SRC racket
  (controller
   ...
   (test (op good-enough?) (reg guess))
   (branch (label sqrt-done))
   (assign new-guess-a (op /) (reg x) (reg guess))
   (assign new-guess-b (op +) (reg new-guess-a) (reg guess))
   (assign new-guess-a (op /) (reg new-guess-b) (const 2.0))
   (assign guess (reg new-guess-a))
   (goto (label test-guess))
   sqrt-done
   )
#+END_SRC

And, to expand on ~improve~, the operations ~*~, ~-~, ~abs~, and ~<~ are assumed primitive.
A pair of registers are then used to help figure out whether ~good-enough?~ is true.

#+BEGIN_SRC racket
  (controller
   (assign guess (const 1.0))
   good-enough?
   (assign guess-a (op *) (reg guess) (reg guess))
   (assign guess-b (op -) (reg guess-a) (reg x))
   (assign guess-a (op abs) (reg guess-b))
   (test (op <) (reg guess-a) (const 0.001))
   (branch (label sqrt-done))
   (assign new-guess-a (op /) (reg x) (reg guess))
   (assign new-guess-b (op +) (reg new-guess-a) (reg guess))
   (assign new-guess-a (op /) (reg new-guess-b) (const 2.0))
...
Of course, as we never use the ~guess-?~ and ~new-gues-?~ registers at the same time, these can be collapsed:
...
#+BEGIN_SRC racket
  (controller
   ...
   (test (op <) (reg guess-a) (const 0.001))
   (branch (label sqrt-done))
   (assign guess-a (op /) (reg x) (reg guess))
   (assign guess-b (op +) (reg guess-a) (reg guess))
   (assign guess-a (op /) (reg guess-b) (const 2.0))
   (assign guess (reg guess-a))
   (goto (label good-enough?))
   sqrt-done
   )
#+END_SRC

I'm not sure register use can be reduced further.
Though, I wouldn't be shocked.

Ok, assigning to the same register one draws from is ok.
So, we can clean things up a little…
...
Note, we need to use ~div~ (or some register other than ~guess~) for the ~good-enough?~ sub-routine, as this is only a test --- ~guess~ itself is not updated.

** 5.4

Just the controller for this exercise, as drawing diagrams is rough…

*** 1.

The controller for recursive ~expt~ is very similar to the controller for ~fact~ in the book.
This is no real surprise.
Though, as we don't use the value of ~n~, there's no need to store this on the stack.
Instead, we just need to ensure the right trail of continues in order to multiply ~b~ by iteself the right number of times.
...
#+BEGIN_SRC racket
  (controller
   ...
   (goto (label expt-loop)) ;; keep in the recursion loop
   after-expt
   (restore continue)
...
The iterative version doesn't use a stack, and is fairly straightforward…
...
~val~ is not part of the stack, but is included to help see what's happening.
Similarly, ~save~ and ~restore~ are marked in the left column, as these happen together.
...
The redudant ~save~ and ~restore~ are commented.
There's no need to read ~continue~ from the stack, make an assignment to ~n~ which does not require ~continue~, and then write ~continue~ back on the stack.
...
Machines have been added to the simulator, and some fixes have been made.
...
~extract-labels~ tells us the way labels are extracted from the text.
Specifically, everything is processed from bottom to top (as ~extract-labels~ is recursive and initialises the return data structure on the base case and then builds through return calls).
So, the /initial/ label will be processed last.
And, if there are no checks in place, will overwrite however the later label was processed in the case of any conflict.

~make-label-entry~ adds the ~cons~ of the label name with (a pointer to) the instructions to the ~labels~ table.
So, there's no conflict.

Instead, we have a label associated with the instructions processed so far.

This is kind of neat.
We're building up the instruction table as we go.
And, all we have is a pointer to the start of the instructions.
So, when we store a label, we're just storing a pointer to the appropriate place in the instructions.
The final result, basically, is a list and a bunch of named pointers to parts of the list.

Labels are idenitified by searching through a the ~labels~ lookup table.
Here, we need to check on how ~assoc~ works.
And, by inspection, this works through the list from head to tail.
So, the first label in the procedure will be found first (a.k.a the last label processed by ~extract-labels~ when returning).

So, ~a~ is assigned ~3~ and that's all that happens.

This can be checked in the simulator.

The modification to check for a reused label has been added to the machine.
In short, this just wraps the label case in the return of ~extract-labels~ in a check through the existing ~labels~.

** 5.9

~make-operation-exp~ is modified to read:
...
In short, for each operand, a check is performed to see whether the operand is a register or a constant.

Here we explicitly whitelist the two options, as along with labels, ~make-operation-exp~ as defined allows operating on ops, etc.

It would be nice to modify ~operation-exp-operands~ as, intuitively, a label (etc.) expression is not truly an operand.
However, this is just a list.
And, I see little motivation for processing the same list twice.

** 5.10

Ideally, you should be able to accomate a new syntax exclusively by adjusting syntax procedures.
For, the only interaction with the syntax for register machines is though syntax procedures.

Though, I don't see sufficient reason to do this.

Renaming things is trivial.
And, something like moving to infix notation seems pointless.
For, there's no reason to assume (or require) the register machine is limited to binary operations.
...
Commented below.
In short, the original code swapped the contents of ~n~ and ~val~ to restore ~val~.
However, given the default behaviour, this isn't required.
...
We modify only ~make-save~ and ~make-restore~.

~make-save~ puts a pair containing the register name and value on the stack.
...
And, ~make-restore~ ensures the register names match before restoring the val.
...
In short, we change the stack to a table of stacks.
Then, for each stack operation we lookup the stack associated with the register.
...
Strictly speaking, the id check for ~pop~ isn't required as an error for the lack of assigned register will be hit first.
Still, I prefer to keep it in.

The difficult thing here was recalling the way to modify the table.

Given ~entry~, etc, it's no use performing ~set!~ as within a lambda this only has local scope.
So, ~set-cdr!~ is the way to go.
This makes the code a little messy, and it would be nice to revist this so the various ~set-cdr!~ commands are more explicit wrt. modifying the stack.
Still, for now things work…

Figuring the above out took some effort, and recalling a cons of a name and a list is printed as a list also helped.

The other task is to set up the table of stacks.
This is done by modifying make machine to create stacks when allocating registers.

** 5.12

The simulator stores the original instruction alonside the compiled instruction.

So, an easy approach to retrieving the relevant information is to parse this.
In particular, as we're dealing with a list and the items of the list follow strict syntactic patterns, this is fairly simple to do by the use of filters.

Specifically, we define an ~inspect~ procedure, which itself expands to a message passing object.

So, added to ~make-new-machine~ is the following:
...
This opens up a message passing procedure which works on the list of original instructions.
The key procedure this relies on is ~filter-unique~.

This returns a list filtered by some predicate with the filtered elements transformed in some way.
And, ensures each transformed element appears in the out-list only once.

The method for ensuring uniqueness is simple for now.
A (not so) quick search through an internally stored list of seen elements.
Though, this could be improved.

Note, it's easy to specify a 'null' transformation with ~lambda (x) x~.
...
To automatically install registers we modify ~assemble~.
In particular, we redefine ~extract-labels~ to ~extract-info~ and extend the procedure to keep track of registers found.
Then, before updating the instructions, we install the registers.

~assemble~ is the following simple edit:
...
Note, there are still only two main cases (label/inst.) but instruction now also scans for registers in the expression, and appends these to the register list if new.

~found-registers~ is simple parsing:
...
And, the append procedures are:

#+BEGIN_SRC racket
  (define (append-new-elem! e l)
    (cond ((null? l) (set! l (list e)))
	  ((equal? (car l) e)
	   #f)
	  ((null? (cdr l)) (set-cdr! l (list e)))
	  (else (append-new-elem! e (cdr l)))))
...
~append-new-elem!~ is not used, but is kept as maybe will use.

This isn't quite as requested.
I don't allocate a register the first time it's seen during an instruction.
But, I think this is a fair way to tackle the problem.
Nothing can happen before the intructions are added, in any case.

Looking back, the request probably works as a hint.
~lookup-register~ in ~make-new-machine~ returns an error if a register is not found.
And, this could be reworked to add the register.

Hm, this seems like it might be better, depending on what one is up to.
For, looking up a register always requires working through the table of registers.
So, adjusting the not-found behaviour is simple.

On the other hand, one doesn't need to assemble the instructions to find out what registers are used.
Though, I don't think this is clearly enough of a benefit…

** 5.14

Given ~n~:
- The number of pushes is ~(* (- n 2) 2)~
- The depth of the stack is ~(* n (- n 1))~

To read the stack information, the instruction ~display-stack-stats~ is defined.
And, the evaluated procedure simply calls the ~print-statistics~ procedure given in the text.
This allows one to inspect the stack at any point in the execution, and so 'current depth' has also been added to the printout.

The modification is an addition to ~make-execution-procedure~ to deal with the syntax and the make command is:
...
Rather than extending the machines interface further, the inspector has been moved to a distinct procedure which takes a machine as an argument.
This can then be easily saved to a variable, etc.
And, this cuts down on the size of the machine procedure.

The procedure is the same as before, and to count executions a counter is added to the machine which is incremented on every execution…

I'm not sure what the use of resetting the instruction count is, so this is a separate message.

** 5.16

Two variations on ~execute~ have been added.

The first executes as many of a specified number of steps as possible.
This can be called directly, similar to ~execute~.

The second is an interactive prompt similar to that of the evaluator which repeatedly asks for a number of steps to be executed and the calls the first variant.

Both of these take a ~trace~ argument and print the step to be executed if requested.

The relevant part of ~make-machine~ is:
...
I ran into a similar problem is pointing towards while attempting to print out the contents of a register before and after executing an instruction.

The issue is, once assembled, labels are implicit.
These are just pointers to some exeuction sequence.

So, in order to represent the label we need to either include information about the label in the assmbeld code, or somehow track the assmebled code.

Tracking is hard.
For, although every execution has the corresponding control instruction, when reading from a register there's no guarantee about when the register was written to.

The actual exercise here seems maybe easier.
For, we just want labels before any instruction.
In particular, ~extract-info~ parses everything an extracts labels.
So, when ~extract-info~ parses a label, this is added to a buffer of labels, which is expanded until an instruction is found and then the contents of the buffer is returned.
Then, ~assemble~ and the associated procedures need to be updated to ensure the contents of the buffer are associated with the procedure.
I.e., we'd have a list of (text assembly before-labels) rather than (text assembly) as it is now.

But, I really don't see the interest in this.

For the register problem, and a more general solution, the idea is to explicitly include labels in the assembled code.

This is fairly easy to do.
For, the only time a label is used is with a ~goto~ statement.
So, that is, to accomodate a different data type for lables, we only need to update ~make-goto~ and ~make-primitive-exp~.

For ~make-primitive-exp~, the label condition returns a tagged list, with the (ironic) ~primitive-label~ tag, the name of the label, and where the label points.
...
For ~make-goto~ we either have a label or the contents of a register.
Only the register case is of interest, as this is the only time a primitive label expression will have been made.
So, we update this to point to the instructions of the primitive label.
...
Finally, the dispatch method for registers is updated to include an option for the additional request.
Here, we add an error to ensure the request is only made when needed.
...
Tracing of registers is kind of already included with the tracing I've implemented.

And alternative way to do this is to modify the ~set~ command.
Internal to the register could be a flag for tracing, and when on print the present contents and the value being set.
...
Why are breakpoints set relative to labels?

Well, okay.
Lots of code and no line numbers, then labels are a good landmark as these are unique.

And, putting breakpoints directly into the control sequence isn't ideal, as then breakpoints can't be dynamically assigned.

So, then, the next issue is whether the nth insturction refers to the nth instruction executed after the given label, or the nth instruction in the controller.
The example in the text does nothing to disambiguate these two cases…

And, it surely matters in some cases.
For, there are plausibly various goto statements between two labels…

So, my guess is it's the nth instruction in the controller.
And, in any case, it's easy to obtain the other behavious by procedures I've already written by going to the label and then inputting the number of executions to perform.

*** storing relative location and modifying ~assemble~

The goal is to store relative location indexed to a (group of) labels along with instruction text.

As we're dealing with labels, ~extract-labels~ or ~extract-info~ is the place to look.
However, the original ~extract-labels/info~ works recursively.

This is very helpful for easily getting the right location for the label to point to in the instruction sequence as we end up processing the text in reverse.

However, this is not very helpful for storing relative position.
For, we have no idea of what labels are to come.

We could try to store information about labels on the way down (so to speak).

But, instead, we could also process everything as we work through the controller text.
We've got a lot of procedures that modify state, so let's add a few more!

~extract-labels/info~ is now merged into ~assemble~ as follows:
  ...
  (let ((text-pointer controller-text)
    ...
    (define (extract-info-loop)
      (cond ((null? text-pointer)
	     ...
	     (for-each (lambda (x) (set-cdr! x (cddr x))) labels)
...
There's a few things to be careful of here.
First, we want to keep ~instructions~ fixed to point to all of the instructions as we need this for ~update-insts!~.

And, we want to append new instructions to ~instructions~.
So, ~instructions~ is 'once shifted' to have a ~nil~ head which remains constant and a tail which adjusts.

(There are other ways to do this, but this functions.)

So, when it comes to ~update-insts!~ we shift all the label points with ~(for-each (lambda (x) (set-cdr! x (cddr x))) labels)~

Further, we also want to find the current end point of ~instructions~ in order to point labels to the right place.
To do this, we simply look to the current terminal point of ~instructions~ with ~end-of~, defined as:
...
~make-instruction~ is updated to take a location arguement and structured as a table:
...
And, related functions are updated to deal with this.

At this point, breakpoints should be ok.
We can search through the instruction sequence for the right location.
And, then wrap the assembled procedure in an additional function.

One issue is the inspector is currently broken.
But, that's for another time.

Also, took the opportunity to rewrite register registration to happen when a register is searched for but not found.

*** injecting a procedure

With instructions tagged, it's now possible to 'inject' a procedure to run before the instruction associated with a label.
...
Most of this is finding the right label.
Then, once this is found, we substitue the procedure for a new procedure which first executes ~proc~ and then executes whatever the original instruction was.

With dispatch updated, we can do things like:
...
And this printout happens before every call to the instruction at ~(afterfib-n-1 . 1)~.

*** beyond what's written

At the moment, we just inject a procedure.

Ideally, this should unfold to a dispatch procedure which processes various messages.

Once message would be to perform the original instruction (i.e. 'proceed').

And, another would return the original instruction.
This could then be used to remove the injection, by setting the cdr of the instruction entry back to the original instruction.

None of this is particularly interesting to me.

Liekwise, ~cancel-all-breakpoints~ would easily implemented given a list of all modified labels.

So, this is where things are left for now.
...
Where ~x~ points to ~p1~ and ~y~ points to ~p2~.

It's not clear to me whether ~p0~ is ever used, as we're just told ~free~ holds the next available index and is incremented by 1.

~y~ is added backwards, as we're still doing everything in applicative order.
And, expanded, ~(list x x)~ reads ~(cons x (cons x nil))~.
So, we first evaluate ~(cons x nil)~ and then do ~(cons x …)~.

Though, I'm not sure whether this really matters.
It seems one could speculatevely assign a pointer and then work on whatever is to be stored at the pointer.
And, as soon as something more complex than a list is considered, there's not going to be much structure to the pointers in either case.

** 5.21

*** 1. Recursive count leaves

The procedure of interest is:
...
Taking ~null?~, ~pair?~ ~car~, and ~cdr~ as primitive, the following register machine works out.
...
It's stated ~null?~ and ~pair?~ check the type of pointer.
So, at the moment I don't think there's anything else to do, other than substituting in the alterantive implementations of ~car~ and ~cdr~.
...
The following is a simple variation on the previous.
The only real difference is ~val~ is set to ~0~ intially, and directly update on the relevant base case.
This means we don't need to save and restore ~val~ from the stack.
...
Goal is a register machine for ~append!~.
...
The following works (and works when ~x~ is empty):
...
I'm not sure whether an optomisation is possible.
We save ~l1~ to test whether ~(cdr l1)~ is ~null?~.
Then, restore everything.
This is done to preserve the state of the stack.
But…

** 5.23

Given we have ~cond?~ and ~cond->if~, the goal is to test for a ~cond~ expression, translate this to an ~if~ expression, and evaluate the resulting expression.

So, to ~eval-dispatch~ we add:
...
And, somewhere in the controller we add:
...
Any other derived expression will be added in a parallel manner.

** 5.24

As a first pass, the logic of ~expand-clauses~ is expanded as follows:
...
In short, ~argl~ is used to store the (relative) first argument, and after the expression is broken down the contents of ~exp~ is no longer needed and so used to store the new expression, which is paired with what's stored in ~argl~.

To pair, however, we need to break ~argl~ into predicate and consequent.
To do this, ~val~ is used.

Tested on some inputs, and I don't see any clear issues, but will return and modify things if something turns up.

E.g. the first exercise can be completed fine:
...
1. The manual evaluator so I could trace where a problem happened.
I didn't pay attention to ~make-if~ and so didn't split pred and conseq.
And, this highlighted when there was register read/write confusion.
(Surprisingly, this only happened once, though there isn't really too much potential for things to go wrong.)

2. (Re-)implementing the constraint on reading and writing only to the same register.
Mostly for ease of verifying things are okay.

** 2.25

Getting a lazy evaluator wasn't so easy…

First, while it is possible to use ~actual-value~ and related procedures from the meta-cricular evaluator, this is far from ideal.
As, the procedures themselves involve ~eval~, which needs to be lazy.
So most of the work would be done by the simulating machine.
And, in contrast to, say, ~let~ this isn't an in principle reduction.
For, ~eval~ is at the core of the machine we're simulating.

So, a primative approach is the way to go.
Or, at least, mostly.
We'll still use ~thunk?~ and some other ops like ~list~ to ease things.

First, ~actual-value~.
Note, on return we update the expression with val, as ~eval~ writes to ~val~ but we've forced something to be evaluated.
...
Then, the very messy part.
The adjustments to ~ev-application~.
In short, we duplicate the operator processing loop for primitive and compound cases.
Primitive works as with the applicative evaluator.
Compound creates thunks, rather than evalauting.

It would be nice to inline the compound test.
However, this then requites ensuring ~proc~ is saved and restored throughout the machine.
I guess this may be the case, but I haven't checked.
Likewise, it may be possible to use a register as a flag with some care, but this seems quite hard to figure out.
In any case, the parallel loops mirror the way lazyness for the meta-circular evaluator is implemented.

The other adjustments are just replacing calls to the evaluator with calls to ~actual-value~.

In hindsight, this isn't too much work, but figuring out the way everything worked and then being sure enough of the approach to work through some bugs (like forgetting to actually test for thunks) was fairly challenging.
...
The maximum depth is a constant 10.

*** 2.

#+BEGIN_SRC racket
  (define (total-pushes n)
    (+ 29 (* 35 n)))
...
These are the results I get for small n.
...
#+BEGIN_SRC racket
  (define (maximum-depth n) (+ 8 (* 5 (- n 1))))
...
Predication, maximum-depth of 60 is 303.
Confirmed.

|---+--------------+-----------------------+---------------|
| n | total-pushes | pushes-alt-mult-of-16 | maximum-depth |
| 1 |           16 |                     1 |             8 |
| 2 |           48 |                     3 |            13 |
| 3 |           80 |                     5 |            18 |
| 4 |          112 |                     7 |            23 |
...
Prediction for 60 is 1904.
Confirmed.
Preducation for 91 is 2896.
Also confirmed.
...
So, this is worse than the factorial given in terms of depth, but still better than recursive.

It seems tail-recursion is important.
In particular, because there's no way to repeatedly do something without calling a procedure.
...
Maximum depth is the same as recursive factorial.
...
So, total pushes is a variation on the fib sequence.
...
#+BEGIN_SRC racket
  (define (total-pushes n)
    (cond ((= n 0) 16)
	  ((= n 1) 16)
	  (else
	   (+ 40 (total-pushes (- n 1)) (total-pushes (- n 2))))))
...
So, we multiply the fibonacci sequence by 16, and the ratio also increases by 40 each time.
...
Right, of course, we want to offset the 40 by one, as this isn't applied to the first (two) element(s) in the sequence.

To prove, go by induction.
We've already covered the base case.

Our goal is to show:

#+BEGIN_SRC racket
  (- (* 56 (fib (+ n 1))) 40) = (+ 40 (total-pushes (- n 1)) (total-pushes (- n 2)))
#+END_SRC

This works out by a bit of substituation, granting the induction hypothesis.
The key observation is going from the fourth to fifth step.
~(+ (fib n) (fib (- n 1)))~ just is ~(fib (+ n 1))~.
...
This is really cool.
In our case, ~a = 16~ and ~b = 40~, but this induction step doesn't rely on the specific values.

Hence, this is a general observation about formulas which capture fibonacci sequences with an initial offset ~a~ and constant addition ~b~:

#+BEGIN_SRC racket
  (mod-fib n) = (- (* (+ a b) (fib (+ n 1))) b)
#+END_SRC
...
So, a key place is ~lookup-variable-value~.
Instead of going directly to whatever is in ~(reg continue)~, we branch to ~unbound-val-error~ if ~unassigned-variable?~ returns true.
...
~unassigned-variable?~ looks to see if the variable points to the empty environment.
...
And, ~lookup-variable-value~ is modified to return the empty environment, rather than an error.
...
#+BEGIN_SRC racket
  ...
  (define (env-loop env)
    ...
    (if (eq? env the-empty-environment)
        env ;; no error signaled here, just returning empty env
        (let ((frame (first-frame env)))
          (scan (frame-variables frame) (frame-values frame)))))
  (env-loop env))
#+END_SRC

The same holds for ~set-variable-value!~.
Where, instead of returning an unbound variable error we return the empty env.

At this point, I'm not really sure what else to do.

It will be work to ensure that there is no other place where a variable is read.
At the moment, this is the only place where it makes sense for a variable to be read.
But, something could be hiding.

Still, various tests don't show anything.

~(* n 1)~ fails.
~(define n 1)~ then ~(define n m)~ and ~(set! n m)~ fail.
~(define b (+ c 1))~ fails.

Though, something that did come up was with ~try~:
...
~(try 0 (/ 1 0))~ returned the unbound variable error rather than failing.
And, the problem was ~/~ wasn't listed as a primitive procedure.

One question is whether this was the right error, before the primitive procedure was added.
In a sense, this seems right.
Any non-primitive procedure is really just a variable pointing to a lambda expression.
But, at the same time it's not clear this error is helpful.

Though, the regualr error checker doesn't distinguish between ~(n 1)~ and ~(+ n 1)~ when ~n~ is unassigned.
So, maybe this is fine.

So, I'm at a loss with this exercise.
It doesn't seem to be lots of work, but I'm not sure what I'm missing…
Perhaps the idea is to rewrite the lookup inside the machine?

*** TODO b.

Figuring out how to test for certain errors seems interesting.
But, on the whole I don't think there's much to be gained from this exercise.
Moving things inside the machine would be nice assmebly practice, but I've got other plans for this.

** 5.31

A table with some justification.

|---------------+------------------------------------------------+----------------------------------+-------------------------+----------------------------|
| procedure     | env operator                                   | env operand                      | argl operand            | proc around op sequence    |
|---------------+------------------------------------------------+----------------------------------+-------------------------+----------------------------|
| (f 'x 'y)     | No need.                                       | No need.                         | No need.                | No need.                   |
|               | No change is made to the environment           | The arguments are characters     | No other argument lists | No other operators         |
|---------------+------------------------------------------------+----------------------------------+-------------------------+----------------------------|
| ((f) 'x 'y)   | No need.                                       | No need.                         | No need.                | No need.                   |
|               | f can do whatever (take us to env where        | The arguments are characters     | No other argument lists | No other operators         |
|               | defined) but we're only looking up chars after |                                  |                         |                            |
|---------------+------------------------------------------------+----------------------------------+-------------------------+----------------------------|
| (f (g 'x) y)  | No need.                                       | Need.                            | Need.                   | Need.                      |
|               | Just looking up lambda in current env          | Don't know what y is             | Will eval (g 'x)        | g is a different proc to f |
|               |                                                | It need the original env after   | And, y may be anything  |                            |
|               |                                                | any changes made by (g 'x)       |                         |                            |
|---------------+------------------------------------------------+----------------------------------+-------------------------+----------------------------|
| (f (g 'x) 'y) | No need.                                       | No need.                         | Need.                   | Need.                      |
|               | Just looking up lambda in current env          | g could require part of the env  | Will eval (g 'x)        | g is a different proc to f |
|               |                                                | is overwritten, etc.             |                         |                            |
|               |                                                | However, what remains is only    |                         |                            |
|               |                                                | to lookup 'y so any overwrite is |                         |                            |
|               |                                                | ok.                              |                         |                            |
|---------------+------------------------------------------------+----------------------------------+-------------------------+----------------------------|

The key thing to think about is the order the various registers are used, and whether or not any subsequent task may make use of the registers.
...
We still need to save the previous continue.
And, we still need to break down the expression.
But, saving ~env~ and ~unev~ is required only if we need to evaluate the operator.

So, we branch on whether the proc is a symbol.

If so, then lookup the symbol.

Else, save ~env~ and ~unev~ and go to ~eval-dispatch~.
Then, take a detour before returning to ~ev-appl-did-operator~ to restore these.

*** b.

This won't eliminate the advantage of compliation altogether as these optimisations only affect the evaluation of instructions.
The constructions themselves remain uncompiled.

Unless the idea is to build in pre-compiled code.
In this case, compilation wouldn't do much better, as the first evaluation effectively functions as a compliation step.
The problem, however, is that this sort of machine is impossible to build, as it would need to contain compiled code for any procedure.

** 5.33

As defined, the only difference between ~factorial~ and ~factorial-alt~ is the order of multiplication.
...
machine code from the original is uncommented and machine code from the alternative is commented.
...
both process the arguments right to left and in short build the corresponding case from the lisp code.

the original needs to save ~argl~ as this has been constructed with ~n~ and processing the recursive call requires working through some arguments.

In contrast, the latter process the recursive call first, so doesn't need to save ~argl~.
However, ~env~ is saved as a procedure is set to be evaluated before non-procedure arguments.

at this pooint, the task is to get the value of the recursive call.

***** after-call14

between the above and ~after-call14~ either a primitive or compiled procedure has been figured out, and we've got the result of the recursive call to consider.
...
The original needs to restore the saved value of ~n~, while the alternative restores the ~env~ and then looks up the value of ~n~.

*** overall

The difference in the compiled code reflects the difference in the procedures.
Specifically, whether or not ~n~ is figured out before ~(factorial (- n 1))~.

And, it's not clear whether there's a significant difference in speed.
As, both procedures need to save and restore a similar number of times.
The only difference is what is saved and restored.
And as this isn't more than a pointer, it seems the procedures should be on par.

Note, both of these are recursive, as ~*~ is the 'final' operation, rather than ~factorial(-alt)~.
...
This is perhaps best seen by searching for ~save~ and ~restore~ as these are the operations which matter when it comes to stack depth.

Both ~continue~ and ~env~ are restored in order to check ~(> counter n)~.
Specifically, to evalauate ~(> counter n)~.
This doesn't extend the stack as this amounts to looking up the value of two variables, and applying a primitive procedure.

Then, on a false branch ~continue~, ~proc~, and ~env~ are saved.
This is due to evaluating the arguments of ~iter~.
However, given tail recursion, the non-last arguments are evaluated is done so within a 'new' environment.
And, as ~+~ is primitive, ~env~ is immediately restored.

Here, ~continue~ and ~proc~ remain on the stack, as we're not done with the sub-arguments.

Though, ~argl~ is added to the stack, as now we need to keep track of the result of the ~+~ op.
Still, ~argl~ is almost immediately restored, as the ~*~ op is primitive.
Then, as the arguments have been evaluated both ~proc~ and ~continue~ are restored.

Here, then, we've covered all the ~save~ and ~restore~ operations, and we've seen any save is restored without the opportunity for additional saves to be made.

What matters, really, is the absence of saves and restores to ~env~ and other registers due to tail recursion.
The values of ~counter~ and ~product~ are 'overwritten' on each loop.

Indeed, with some hand tuning we can further optimise, as we know in advance everything but call to ~iter~ is primitive.
Hence, there's no need for the first pair of saves and restores with respect to the test, nor is there any need for saves and restores to ~continue~ and ~env~ when evalauting the first argument of ~iter~.

The only saves we really need are with ~proc~ and ~argl~.

*** tuned

To test the above a modification of ~compile-and-go~ which takes machine code is defined.
...
#+BEGIN_SRC racket
  (go-compiled compiled-fact-iter-tuned)
#+END_SRC

where ~compiled-fact-iter-tuned~ is defined as below.
(The saves and restores are removed.)
Things work fine!

*** compiled-fact-iter-tuned

#+BEGIN_SRC racket
  (define compiled-fact-iter-tuned
  '((env continue)
    (val)
    (assign val (op make-compiled-procedure) (label entry1) (reg env))
    (goto (label after-lambda2))
...
The compiled code is equivalent to that obtained from:
...
Below are the two sequences.
Then, a quick diff tool is defined and we see only the label names differ.

(Process of figuring this out amounted to tracing through the procedures looked up and then a little trial and error to get the order right.)
...
This procedure works through two lists, and prints the ~car~ of both when not equal.
  ...
  (define (diff a b)
    (cond
      ...
      ((and (null? a) (not (null? b)))
	   (display "list b is longer than list a"))
	  ((and (null? b) (not (null? a)))
	   (display "list a is longer than list b"))
...
#+begin_example
Difference found!
left  reads: (assign val (op make-compiled-procedure) (label entry16) (reg env))
right reads: (assign val (op make-compiled-procedure) (label entry1) (reg env))

Difference found!
left  reads: (goto (label after-lambda15))
right reads: (goto (label after-lambda2))
...
Difference found!
left  reads: (branch (label primitive-branch19))
right reads: (branch (label primitive-branch3))

Difference found!
left  reads: compiled-branch18
right reads: compiled-branch4

Difference found!
left  reads: (assign continue (label after-call17))
right reads: (assign continue (label after-call5))

Difference found!
left  reads: primitive-branch19
right reads: primitive-branch3

Difference found!
left  reads: after-call17
right reads: after-call5

Difference found!
left  reads: (branch (label primitive-branch22))
right reads: (branch (label primitive-branch6))

Difference found!
left  reads: compiled-branch21
right reads: compiled-branch7

Difference found!
left  reads: (assign continue (label after-call20))
right reads: (assign continue (label after-call8))

Difference found!
left  reads: primitive-branch22
right reads: primitive-branch6

Difference found!
left  reads: after-call20
right reads: after-call8

Difference found!
left  reads: (branch (label primitive-branch25))
right reads: (branch (label primitive-branch9))

Difference found!
left  reads: compiled-branch24
right reads: compiled-branch10

Difference found!
left  reads: primitive-branch25
right reads: primitive-branch9

Difference found!
left  reads: after-call23
right reads: after-call11

Difference found!
left  reads: after-lambda15
right reads: after-lambda2
...
To change this we look at where the argument list is constructed.

Specifically, ~construct-arglist~ and by extension ~code-to-get-rest-args~.

Instead of reversing the operand codes, we work through these as given.
...
~code-to-get-last-arg~ is misnamed in this case.
We're taking the first argument, rather than the last, as the list isn't reversed.

But, the idea is the same:
Start constructing a list.

This is separated from the other arguments as the op involves creating the list.
While, for the remaining arguments we'll append elements to the list.
...
Efficiency is depends on the way ~append~ is implemented.
But, given the memory structure set out before, this is likely costly as we'll need to replicate the standard ~append-elem~ procedure.

An similar case to consider is ~cond->if~.
This is constructed so arguments are evaluated left to right.
Switching the order would break implicit testing of earlier conditions by later, and also else.

So, it's reasonable to expect a user to consider argument evaluation in general happens the same way.
And, if there are intended side-effects…

I guess there's probably /some/ cases where append-elem comes out faster.

Is there an alternative to ~append-elem~?
Maybe.
We just need to keep a pointer to the last element in the list.
This would involve saving and restoring the pointer.
But, that's not too bad compared to a search every time.

** 5.37

The change to preserving is simple.
...
But, I think I've done sufficient save/restore chekcing for now.
(See Ex. 5.34)

** 5.38
*** 1.

I'm not too sure whether this was the expectation.
In any case…

The proc begins with checks to ensure there are exactly two arguments.

Then, the code for arg1 is executed.
This is saved to ~val~.
This is saved.
Then, work through everything for the second argument, and save the result to ~arg2~.
At this point, we have the desired reuslt for the first argument in ~val~, which was restored after setting ~arg2~.
And, copying ~val~ to ~arg1~ is safe with respect to ~arg2~.

This works recursively as we're only interested in getting the right results in ~arg1~ and ~arg2~ at any given time.
At the start, the contents don't matter, and after the result of evaluting the first argument is stored, there's no need to preserve either register.
...
#+BEGIN_SRC racket
  (define (spread-arguments operand-codes)
    (cond ((or (null? operand-codes)
	  ...
	  (else
	   (append-instruction-sequences
	    (cadr operand-codes) ;; figoure out the second
	   (preserving
	    '(val) ;; will save val to arg2 after
	    (append-instruction-sequences
	     (cadr operand-codes) ;; figure out first
	     (make-instruction-sequence
	      '(val)
	      '(arg1)
	      '((assign arg1 (reg val))))) ;; second saved to arg1
	    (make-instruction-sequence
	     '(val)
	     '(arg2)
	     '((assign arg2 (reg val))))))))) ;; val is restored, save to arg2
#+END_SRC

Note, initial version eval'ed the first argument then the second.

On reflection, to keep consistency with the way a list of arguments is evaluated this is switched.
The above is current, but the code generated below has the initial order.

*** 2.

For any open-coded primitive we have test and application procedures as:
...
E.g., a case for addition is written:
...
The open-coded is for sure shorter.

**** original

#+BEGIN_SRC racket
  (text assign val (op make-compiled-procedure) (label entry1) (reg env))
  (text goto (label after-lambda2))
  (text assign env (op compiled-procedure-env) (reg proc))
  (text assign env (op extend-environment) (const (n)) (reg argl) (reg env))
  (text save continue)
  (text save env)
  (text assign proc (op lookup-variable-value) (const =) (reg env))
  (text assign val (const 1))
  (text assign argl (op list) (reg val))
  (text assign val (op lookup-variable-value) (const n) (reg env))
  (text assign argl (op cons) (reg val) (reg argl))
  (text test (op primitive-procedure?) (reg proc))
  (text branch (label primitive-branch6))
  (text assign continue (label after-call8))
  (text assign val (op compiled-procedure-entry) (reg proc))
  (text goto (reg val))
  (text assign val (op apply-primitive-procedure) (reg proc) (reg argl))
  (text restore env)
  (text restore continue)
  (text test (op false?) (reg val))
  (text branch (label false-branch4))
  (text assign val (const 1))
  ...
  (text save proc)
  (text save env)
  (text assign proc (op lookup-variable-value) (const factorial) (reg env))
  (text save proc)
  (text assign proc (op lookup-variable-value) (const -) (reg env))
  (text assign val (const 1))
  (text assign argl (op list) (reg val))
  ...
  (text assign argl (op cons) (reg val) (reg argl))
  (text test (op primitive-procedure?) (reg proc))
  (text branch (label primitive-branch9))
  (text assign continue (label after-call11))
  (text assign val (op compiled-procedure-entry) (reg proc))
  (text goto (reg val))
  (text assign val (op apply-primitive-procedure) (reg proc) (reg argl))
  (text assign argl (op list) (reg val))
  (text restore proc)
  (text test (op primitive-procedure?) (reg proc))
  (text branch (label primitive-branch12))
  (text assign continue (label after-call14))
  (text assign val (op compiled-procedure-entry) (reg proc))
  (text goto (reg val))
  (text assign val (op apply-primitive-procedure) (reg proc) (reg argl))
  (text assign argl (op list) (reg val))
  ...
  (text restore continue)
  (text test (op primitive-procedure?) (reg proc))
  (text branch (label primitive-branch15))
  (text assign val (op compiled-procedure-entry) (reg proc))
  (text goto (reg val))
  (text assign val (op apply-primitive-procedure) (reg proc) (reg argl))
  (text goto (reg continue))
  (text perform (op define-variable!) (const factorial) (reg val) (reg env))
  (text assign val (const ok))
  (text goto (reg continue))
#+END_SRC
...
#+BEGIN_SRC racket
  ...
  (text save val)
  (text assign val (const 1))
  (text assign arg2 (reg val))
  (text restore val)
  (text assign arg1 (reg val))
  (text assign val (op =) (reg arg1) (reg arg2))
  (text test (op false?) (reg val))
  (text branch (label false-branch4))
  (text assign val (const 1))
  (text goto (reg continue))
...
Huh, interesting.
As we're reducing things to binary operations, it makes sense to rewrite the epxression.

The following two procedures do this in a simple way:
...
Of course, this doesn't work with subtraction, as the folliwing are not equivalent:
...
In any case, the task is only to do ~+~ and ~*~ and the updated entries in ~compile~ are;
...
#+BEGIN_SRC racket
  ...
  (text assign val (op +) (reg arg1) (reg arg2))
  (text save val)
  (text assign val (const 3))
  (text save val)
  (text assign val (const 2))
  (text save val)
  (text assign val (const 1))
  (text assign arg2 (reg val))
  (text restore val)
  ...
  (text assign val (op +) (reg arg1) (reg arg2))
  (text goto (reg continue))
  (text perform (op define-variable!) (const f) (reg val) (reg env))
  (text assign val (const ok))
  (text goto (reg continue))
#+END_SRC
...
~lexical-address-lookup~ and ~lexical-address-set!~ both take an address.
And, this address is flexible.
All that matters is that the address stored can be used for lookup.

So, first, is a variation on ~lookup-variable-value~ which traces the address of a lookup and then uses ~lexical-address-lookup~ to display the value.

Then, the two procedures are given.

*** lookup-variable-value-trace

In short, the address is calculated from (0 . 0), left is +1 each time new env is set for scanning, and right is fresh on each env, +1 for every var/val scanned.

Various ~cond~ instances allow the tracing to be hidden.
...
*** lexical-address-… lookup & set!

Both ~lookup~ and ~set!~ share a common core of going to and address.
However, after some headache (see next ex) these been set to individual procs.

Both use ~do-n~, which does some proc n times to something.
...
Lexical addresses are given by:
...
A first attempt claimed this was all straightforward.
I retract this.

There are two issues.

First, the difference between run-time, compile-time and global envs is super important.
These all share the same structure, but what's present in compile-time won't match what's in global, and it's easy to get distracted by global when investigating run-time.

Second, the run-time enviroment isn't amenable to general compile time analysis.
The most significant difficulty I had was attempting to test everything with the cube-root proc.
This has a bunch of nested definitions, and these don't work well with what's asked in these exercises (hence Ex. 5.43, I guess).

The problem is definitions update the environment, but do so dynamically.
That is, at least as far as the compiler is concerned.
Hence, lexical lookups are often to the wrong place.
E.g., if made before definitinos take place.

There's no problem updating the compile-time environment when this happens (just parallel the manipulation of the main env).
But, this is incompatible with the static result of lexical addressing.

Now, all of this is clear if you pay enough attention to the way the compiler is set up, and don't think of definitions as assigning lambda expressions to variables.
But, I didn't pay super close attention, and I only had a handful of hours sleep.
And, when you're testing to ensure you haven't broken anything unexpected, this issue does come up, and it's not obvious from the ex that things are kind of broken for a moment, well…

Though, looking at this /again/ I'm still very puzzled.
The issue is definitions adjust the enviroment.
And, we haven't implementated a way to either separate or secure the compile time address.
Working with internal definition (Ex. 5.43) won't address this issue.
Unless all given definitions are treated this way.
The easiest way to do this is to implement elimination of internal definitions and then wrap any compiled procedure via:
...
The compile time environment is added as 'ct-env' to the end of the relevant procedures.
And, ~compile-lambda-body~ already extracts the relevant variables, so the only task is to add these to the env.

A compile time enviroment is a list of lists of variables.
I.e. the variable fragment of an enviroment.
...
Given the testing with ~lookup-variable-value~, ~get-lexical-address~ (rather than the less descriptive ~find-variable~) parallels the method used to trace a variable, and returns the result of the trace.
...
~compile-variable~ and ~compile-assignment~ are fairly simple.
Note, it's not easy to inline the test as we're writing assembly.
...
*** Huh, scan-out-defines (as defined above) and mutation

~scan-out-defines~ is defined as in Ex. 4.16.
The following is evaluated:

#+BEGIN_SRC racket
  (define (interrupt proc arg)
    (proc arg))

  (define exp0 '(define (f x)
		  (define a 4)
		  (define b 5)
		  (+ a b x)))

  (define exp1 (scan-out-defines '(define (f x)
				    (define a 4)
				    (define b 5)
				    (+ a b x))))


  (scan-out-defines exp0)
  (interrupt scan-out-defines exp0)
  exp1
#+END_SRC
...
#+BEGIN_SRC racket
  (define f (lambda (x) ((lambda (b a) (begin (set! a 4) (set! b 5) (+ a b x))) '? '?)))
  (define f (lambda (x) (set! a 4) (set! b 5) (+ a b x)))
  (define f (lambda (x) ((lambda (b a) (begin (set! a 4) (set! b 5) (+ a b x))) '? '?)))
#+END_SRC


And, now a slight change:
...
#+BEGIN_SRC racket
  ...
  (define exp0 '(define (f x)
		  ...
		  (define b 5)
		  (+ a b x)))
  (define exp1 (scan-out-defines exp0))
...
The problem is the ~scan-out-defines~ given is destructive.
Rather than recreating an expression from scratch, the original expression is modified, and then a variation using the modification is returned.
Maybe not the best practice…


~compile-and-go~ now reads as follows:
...
The option for showing information can be ignored here.
The important stuff happens at the start.

~data~ is the result of two procs. applied to the epxression.
First, ~scan-out-defines~, which works as before.
And internal definitions are scanned out.

~skim-defines~ is where the interesting work happens.
This takes any 'top level' defines and changes these to ~set!~ expressions.
Returned is a pair consisting of the variables for the definitions on the ~car~ and the new expressions on the ~cdr~.

With this we can set up an initial environment where compiled defitions are treated to lexical addressing.

As immediately follows, we have the scanned expressions, and then the initial compiled time enviroment is built any found variables.

Indeed, this is all that needs to happen.

With one exception.
A sequence needs to be marked with ~begin~.
(This was so difficult to track down!)

~skim-defines~ is not too interesting:
...
At this point the following compiles are works fine:

#+BEGIN_SRC racket
  (compile-and-go 2 '(define (factorial n) (if (= n 1) 1 (* (factorial (- n 1)) n))))
#+END_SRC

The scanned expression is:

#+BEGIN_SRC racket
  (set! factorial (lambda (n) (if (= n 1) 1 (* (factorial (- n 1)) n))))
#+END_SRC

The following complies and does /not/ work fine:
...
The only difference is switching the order of multiplication.

The problem is somewhere in open coding.
Surprisingly, this isn't related to tail recursion.

The fix was to preserve the enviroment.

~spread-arguments~now reads:
...
#+BEGIN_SRC racket
  ...
  (cond ((or (null? operand-codes)
	...
	(else
	 (preserving '(env)
		     (cadr operand-codes) ;; figure out the first
		     (preserving
		      '(val)
		      (append-instruction-sequences
		       (car operand-codes)
...
Currently evaluates to ~20~, while it 'should' evaluate to ~-4~.
...
#+BEGIN_SRC racket
  ((lambda (a b) ((lambda (+) (begin (set! + -) (+ a b))) (quote ?))) 4 4)
#+END_SRC

And evaluating this returns ~0~.

*** idea

The task is to get the open-coding compiler to consult the compile-term enviroment.

At the moment, the ct-env doesn't contain any primitives.
So, the idea is to:

1. Implement a list of operations for open-coded primitives similar to ~eceval-operations~.
2. Rewrite the way the environments are handed.
   - Instead of setting up ~ct-env~ and ~the-global-environment~ separately, a variant of ~setup-environment~ can do both.
     - Specifically, after the existing setup, both envs are extended with the open-coded primitives and returned as a pair.
       - A general method which takes this pair and extends could be used here and also for the initial definitions.
3. Update ~compile-and-go~ to work with the above.

So, the result of this is open-coded primitives are handed exactly the same way as any variable known at compile time.
This is exactly the way top level definitions are handled in the previous exercise.

The siginifcant difference is that rather than being initially unassigned, these procedures have something set.

There is a problem with this apprach.
At run-time the ct-env remains.
Hence, when looking up a variable, the open-coded instance may be found first.
So, we'd like to 'hide' the primitives behind the global environment.
This can be done by setting a corresponding empty env in the ct-env.

The downside of this is that primitives will turn up in a search for a variable at run-time.
I kind of like this, though.
You can specify the primitive operations that the machine does, simulated by the compiler.
Then, a variety of shortcut procedures are added to the global enviroment.
And, finally, a collection of inital definitions are set up.
So, you can use this to start eliminating the shortcut procedures.

Though, this is very limited.
There's no similation of memory.

*** write-up

The first thing we do is split ~eceval-operations~ into open- and closed- variants.
~open-eceval-operations~ will be used for open-coding while ~closed-eceval-operations~ are not.
All of these ops are available to the machine, but open-coded-ops are also part of the compile time environmnet.
And, in principle, could be part of any environment.

To help keep the global and compile time environents in sync, we work with a pair.
To extend the pair, we have a procedure which optionally skips the compile time environment.
Specifically, this is used to skip lexical addressing of the global environment.
...
The setup for the pair the begins with the open-coded operations (available to both envs) and then adds the primitive procedures (only avaiable at run-time).
...
~compile-and-go~ now uses env pairs:
...
At this point, open-coded ops are overwritten when an operation with the same symbol is specified as a primitive-procedure.

So, given the adjustments made in the previous exercise, primitive-procedures should maybe be considered definitions in waiting.
If 'brought forward' then they need to be written using other primitive-procedures or open-coded primitives.

This ends up being somewhat intermediate, then.
The only significant reason to use primitive-procedure +, e.g. is due to this taking an arbitrary arity.
This potentially saves a lot of work, and so perhaps two versions should be defined.
This would be easy to work with in the compiler, as we'd just need to test the arity.
Anyway, for the moment this works as required.

(Note, as we do not block access to primitive procedures at run time, these are availalbe for lookup.
However, as they are part of the environemnt, they are evaluated as reguar procedures.)

** 5.45

*** 1.

Note, these comparisons are with the 'base' ece compiler.
...
#+BEGIN_SRC racket
  ...
  (define (total-pushes n) (* 16 (- (* n 2) 1)))
#+END_SRC

The compiled version performs as follows:
...
#+BEGIN_SRC racket
  (define (maximum-depth n) (+ 2 (* 3 (- n 1))))

  (define (total-pushes n) (+ 1 (* n 6)))
...
To get the ratio of max depth, we can use the following.
Though, it's clear it's going to be roughly ~(/ 5 3)~ as the addition will eventually matter little.

#+BEGIN_SRC racket
  (define (max-depth-ratio n)
    (/
     (+ 8 (* 5 (- n 1))) ; no optimisation
     (+ 2 (* 3 (- n 1))) ; optimisation
     ))

  (* (max-depth-ratio 9999) 1.0)
  (* (/ 5 3) 1.0)
#+END_SRC
...
For total pushes, we have the following.

#+BEGIN_SRC racket
  (define (total-pushes-ratio n)
    (/
     (* 16 (- (* n 2) 1)) ; no optimisation
     (+ 1 (* n 6)) ; optimisation
     ))
#+END_SRC

With a rewrite, it's easier to see the ratio is rougly ~(/ 32 6)~:

#+BEGIN_SRC racket
  (define (total-pushes-ratio n)
    (/
     (- (* 32 n) 16) ; no optimisation
     (+ 1 (* n 6)) ; optimisation
     ))

  (* (total-pushes-ratio 9999) 1.0)
  (* (/ 32 6) 1.0)
#+END_SRC
...
I might retrun to this.

I have a felling optimising the order of intructions might help.
My tendancy is to write explicit ~goto~ insturctions by hand to emulate function calls.
And, there may be a way to structure everything to avoid reduandant saves and restores to continue.

My guess would also be to look at where protection happens.
This is a significant source of save/restore.

Though, going back an exploring this in detail is something for a (different) rainy day.
...
Maximum depth is the same as factorial (no surprise, given both recurse on n - 1).

To figure out the total number of pushes, we can use ~mod-fib~ as proposed in Ex. 5.29.

#+NAME: def:mod-fib
#+BEGIN_SRC racket
  <<def:fib>>
  (define (mod-fib a b n)
    (- (* (+ a b) (fib (+ n 1))) b))
#+END_SRC

The result is an initial offset of ~7~ (versus ~16~) and a constant addition of ~3~ (rather than ~40~).

#+NAME: def:mod-fib
#+BEGIN_SRC racket
  (mod-fib 16 40 4) ;; without optimisation
  (mod-fib 7 3 4)   ;; with optimisation
#+END_SRC

This should give a ration of roughly ~(/ 56 10)~ i.e. ~5.6~.

For the ratio of total pushes:

#+BEGIN_SRC racket
    <<def:mod-fib>>
    (for-each (lambda (n) (display (* (/ (mod-fib 16 40 n) (mod-fib 7 3 n)) 1.0)) (newline))
	      (list 1 2 4 8 16 32))
#+END_SRC
...
Of course, this isn't a ratio in terms of ~n~ alone, but rather ~fib~.

Still, this is coincidently quite close to the result obtained for factorial.

** 5.47

The difficulty present is to access interpreted procedures control needs to be handed over to the simulator.
There's no way to know what's set to be defined ahead of time.

Compiled code more-or-less tracks path that would be taken through the simulator.
In particular, ~compile-procedure-call~ and the following procs closely parallels ~apply-dispatch~.
The difference being the lack of a ~compound-apply~ parallel.
~compound-apply~ goes to ~ev-sequence~, and this in turn may call only interpreted procedures, etc.

So, we need to hand over evaluation of everything to the simulator at this point.

This is simple to do with the additional register via ~(goto (reg compapp))~.

The solution here is complicated a little due to the compiler being set up to optimise while the simulator is not.
In other words, it's important to explicitly save the contents of the ~continue~ register before giong to ~compound-apply~.
But, that's all.

~compound-proc-appl~ is as follows:
...
This parallels ~compile-proc-appl~, but saves continue and goes to ~compound-apply~ (rather than paralleling ~compiled-apply~).

And ~compile-procedure-call~ is as follows:
...
A compiled branch is added, along with a corresponding parallel instruction sequence.

** 5.48

*** Sketch

In the simulator, add a new test for ~compile-and-run~.
Passing this test is a new ~eceval-op~, ~compile~.
Put this into the ~val~ register.
Then, goto the ~val~ register.

At this point, the compiled code is read.
And defs will be added to the relevant env, and any procs executed, etc.

*** Overview

To start, the procedures the simulator calls:
...
#+BEGIN_SRC racket
  ...
  (define (compile-and-run-exp exp)
    (cadadr exp)) ;; remove tag and then remove quote
#+END_SRC

~compile-and-run~ is just ~compile-and-go~ cut down to return ~instructions~.
Having ~eceval~ hardcoded is fine for the given purpose, as the proc is only going to be called with ~eceval~ defined.
Though, for something more general it would be nice to tie dynamically link the simulator ~compile-and-run~ is called from and the argument to the compiler.

~eval-dispatch~ is extended with:
...
And to the compiler we (also) add:
