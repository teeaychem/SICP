...
So, this is split to a separate procedure.
Internally, ~do-n~ is a general procedure to do something to something n times.
...
And, ~compile-lambda-body~ already extracts the relevant variables, so the only task is to construct a list of /something/ to function as values.
Here, we use '*lex-addr-val*'.

At the moment I see no interest in distinguishing the compile time environment from the environments already in use.
The only downside is carrying around 'empty' values.
