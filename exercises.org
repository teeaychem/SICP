...
I guess this is probably quite a nice safety feature.
An additional option would be to scan the stack for the next instance of the register.
Though, if we can do this it's no longer 'just' a stack we're requiring.
Not to mention this introduces a different path for typos.
