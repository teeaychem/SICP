...
#+BEGIN_SRC racket
  (define (let*->nested-lets exp)
    ...
    <<def:let-body>>
    <<def:make-let>>
    (define (nested-lets var-terms var-values body)
      (if (null? (cdr var-terms))
	  (make-let (cons (car var-terms) (car var-values)) body)
	  (make-let (cons (car var-terms) (car var-values)) (nested-lets (cdr var-terms) (cdr var-values) body))))
    (nested-lets (let-var-terms exp) (let-var-values exp) (let-body exp)))
#+END_SRC
...
#+BEGIN_SRC racket
  <<def:let*->nested-lets>>
  (define test (list 'let* (list (list 'a 1) (list 'b 2)) (list '* 'a 'b)))
  test
