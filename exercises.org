...
(define (spread-arguments operand-codes)
  (cond ((or (null? operand-codes)
        ...
        (else
         ...
         (preserving
          '(val) ;; will save val to arg1 after
          (append-instruction-sequences
           (cadr operand-codes) ;; figure out second
           (make-instruction-sequence
            '(val)
            '(arg2)
            '((assign arg2 (reg val))))) ;; second saved to arg2
          (make-instruction-sequence
           '(val)
           '(arg1)
           '((assign arg1 (reg val))))))))) ;; val is restored, save to arg1
#+END_SRC
...
Ran into something odd.
~abs~ isn't ok in the ece simulator.
