...
Finally, the dispatch method for registers is updated.
To ensure ~get~ functions as before, we make sure this ignores the extra label wrapping.
~get-raw~ shows the 'true' contents.
And, ~get-simple~ is like ~get~ but shows the label name rather than the label pointer.
